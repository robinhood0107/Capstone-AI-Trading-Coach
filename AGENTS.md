...
- 설정 파일을 제외한 코드에는 필요한 지점마다 한글 주석을 남긴다. 주석은 각 언어의 기본 주석 문법(`#`, `//` 등)을 쓰고, `# 계약/보안/운영 관점에서 이 선택이 필요한 이유`처럼 별도 접두어 없이 바로 적는다. 코드가 무엇을 하는지 반복하지 말고 왜 그런 설계가 필요한지를 짧게 설명한다.
