...
- 커밋과 PR에 AI 도구(assistant, workflow 등)를 author/co-author로 표기하지 않는다. `Co-Authored-By` trailer와 AI 서명 footer를 넣지 않으며, contributors에는 사용자 계정만 남긴다.
...
## gstack / workflow / assistant 사용 규칙
...
- 사용자가 workflow native browser, web search, connector, app-specific tool을 명시하면 그 명시를 우선한다.
- assistant 전용 설정(`.local-tool/` hook, assistant-only MCP, assistant-only command)은 사용자가 별도로 요청하기 전까지 추가하지 않는다.
- `RULES.md`는 이 파일을 따르는 짧은 연결 문서로만 유지한다.
...
- 설정 파일을 제외한 코드에는 필요한 지점마다 한글 주석을 남긴다. 주석은 각 언어의 기본 주석 문법(`#`, `//` 등)을 쓰고, 별도 접두어 없이 바로 적는다.
- 주석은 코드가 이미 말하는 “무엇을 하는가”를 반복하지 말고, 계약·보안·운영·테스트 관점에서 “왜 이 방식이어야 하는가”를 1~2문장으로 설명한다. 예: `# 휴장일에는 외부 KIS 호출 자체를 만들지 않아 rate limit과 장애 전파를 동시에 줄인다.`
