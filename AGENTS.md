...
- 구현 또는 구현 계획 세션을 시작할 때는 `private-reference/agent/10_AI협업_컨텍스트_프라이머.md`를 먼저 확인한다. 이 프라이머의 문서 지도, 세션 프롬프트 템플릿, 과거 실수 기록을 사용해 사용자가 매번 같은 컨텍스트를 다시 설명하지 않게 한다.
- 프라이머 확인 뒤에는 현재 세션에 해당하는 `private-reference/agent/팀원1_보강검토_및_세션별_작업계획.md`의 작업, DoD, 함정과 대비책을 확인한다. Decision/Risk/Order/RAG처럼 세부 설계가 필요한 작업은 프라이머의 문서 지도에 따라 상세 구현명세서, ADR, 권장 코딩패턴 예시집을 추가로 읽는다.
- 세션 운영 판단은 `private-reference/agent/팀원1_보강검토_및_세션별_작업계획.md` 10장의 운영 팁을 따른다. 특히 S6.4(BSM/Greeks/IV)는 막힌 날 사용할 버퍼 세션으로 보고, Kafka 트랙은 S7.0+S7.3만으로 시연·계약·보고서가 성립한다는 기준 아래 매몰비용을 경계한다.
...
- 설정 파일을 제외한 코드에는 필요한 지점마다 한글 주석을 남긴다. 주석은 `// 계약/보안/운영 관점에서 이 선택이 필요한 이유`처럼 `//` 뒤에 바로 내용을 쓰고, 별도 접두어는 붙이지 않는다. 코드가 무엇을 하는지 반복하지 말고 왜 그런 설계가 필요한지를 짧게 설명한다.
