...
아래는 시연용 스택(`capstone-p1-demo`)에서 그대로 캡처한 것입니다. 2026-06-15 부터
2026-09-18 까지 75세션을 채워 둔 상태이며, 숫자는 전부 DB 에 저장된 일봉과 주문 기록에서
계산됩니다. 화면에 채워 넣은 값은 없습니다.

### 자동운용 — 예산, 포지션, 실제 체결

![자동운용 화면](docs/screenshots/automation.png)

보유 종목과 ATR 기반 청산선, 그리고 그날의 판정을 함께 봅니다. 오른쪽 실행 이력에서
**2026-09-16 삼성SDI 매수 45주 전량 체결**처럼 실제로 나간 주문을 확인할 수 있고, 각
줄의 「판단 근거 보기」로 왜 그 주문이 나갔는지 또는 왜 후보가 없었는지 열어 봅니다.

### 백테스트 — 원칙을 켜면 무엇이 달라지는가

![백테스트 리포트](docs/screenshots/backtest.png)

같은 구간에 원칙 개입 정도만 바꿔 셋을 돌립니다. 원칙 없는 Baseline 대비
**최대낙폭 34.7%**, **꼬리손실(CVaR) 54.9%** 가 줄었습니다. 대신 연환산 수익률을 5.1%
포기했습니다 — 개선만 보여주지 않고 대가를 같이 적습니다. 표본이 24거래일로 짧아
우열을 단정하지 않는다는 문장도 화면에 그대로 둡니다.

### 금융 Agent — 저장된 질문과 답변

![금융 Agent 답변](docs/screenshots/agent-answers.png)

개념을 물으면 근거를 붙여 답하고, 그 질문과 답변은 계정에 남습니다. 위는 금 선물 ETF의
롤오버 비용과 콘탱고를 물었을 때의 실제 답변입니다. 답변마다 「도움 됨 / 아니오」로 평가를
남기고 「학습일지로 남기기」로 자기 기록에 옮길 수 있습니다.

### 주문 검토 — 열네 개 규칙의 판정

![주문 검토](docs/screenshots/order-review.png)
...
평가는 09:30 KST, 신규 주문 마감과 미체결 취소·대사는 15:20 KST입니다.
PC 절전 복귀 뒤에는 실제 현재 시각을 다시 확인합니다. KIS 모의 서버의 TLS 연결이 되지 않으면
당일 실행을 생성하지 않고 장중에 연결을 다시 확인합니다.
Dashboard의 **자동운용** 화면에서도 봇의 control 상태, blocker, 실행 기록과 보유 포지션을 확인할 수 있습니다.

당일 데이터 장애로 종료됐고 아직 종목 선택·주문 예약·주문 제출이 전혀 없다면, 연결 복구 뒤
최대 두 번 자동 재시도합니다(첫 실패 2분 뒤, 다음 실패 5분 뒤; 15:20 이전).
`./capstone mock resume`도 같은 두 번의 한도를 공유합니다. 정상 종료·위험 차단·주문 제출 뒤에는
재시도하지 않습니다. 같은 실행 ID와 호출 누적값을 유지하며 DB 트랜잭션으로 중복 재개를 막습니다.
기본 프로필의 1회 주문금액 제한은 비활성화돼 있습니다. 총 운용자금·잔고·보유 수량·일일 주문 수
검사는 유지됩니다.
