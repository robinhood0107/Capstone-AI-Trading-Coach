...
- `flyway` role은 PostgreSQL 일반/오류 parameter logging 길이를 모두 `0`으로 고정한다. V7 bootstrap과 rotation은 공통 verifier로 credential bind 전 두 effective setting을 검증하고 안전하지 않으면 mutation과 audit 없이 fail-closed한다. 기존 volume은 V7 전에 role 설정을 명시적으로 재적용하며 SQL 문장 로깅 자체는 허용한다.
...
- The `flyway` role pins both normal and error PostgreSQL parameter-log lengths to `0`. V7 bootstrap and rotation share one verifier that checks both effective settings before any credential bind and fails closed without mutation or audit when either is unsafe. Existing volumes must reapply the role settings before V7; SQL statement logging itself remains permitted.
