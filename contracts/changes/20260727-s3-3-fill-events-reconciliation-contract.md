...
9. **P0-19 — offline 기본값과 비범위**
   ...
   - S3-online은 `VTTC0081R`/`VTSC9215R` strict parser/read를 exact 5단계 approval probe의
     ...
     자동 DB 반영은 추가하지 않는다.
   - exact KIS_MOCK `FULL` packet은 선택적 `exchangeDivision`을 가질 수 있지만,
     현금 신규주문 provider handoff는 KIS Developers 계약상 `KRX`만 허용한다.
     `NXT`는 packet/online transport 검증에서 fail-closed 하며, 주문 submit,
