...
- `kis-mock-brokerage-approval-author`는 current clean worktree, `origin/<branch>`, GitHub PR
  number/base/head/required checks, Redis limiter PTTL을 직접 읽는다. operator가 SHA나 check
  conclusion을 입력할 수 없다. output은 기존 owner-only mode `0700` directory의 absolute path에
...
- `CANCEL_RECOVERY`는 source approval ID/SHA/nonce에서 만든 non-secret anchor가 encrypted
  `COMMITTED` reference와 일치할 때만 허용한다. source `cancelFull` 실패는
...
- The author reads the clean worktree, `origin/<branch>`, GitHub PR/base/head/checks,
  and Redis PTTLs directly. It only creates a new owner-private `0600` regular file
...
- `CANCEL_RECOVERY` can only reuse the encrypted `COMMITTED` reference anchored to
  the original FULL packet. It cannot express a new submit. A failed cancel permits
