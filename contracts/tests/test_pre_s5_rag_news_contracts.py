...
class PreS5RagNewsContractTest(unittest.TestCase):
    ...
    def test_voyage_vertex_and_generation_boundaries_are_contract_only(self) -> None:
        ...
        self.assertEqual("VERTEX_MODEL_ID", vertex["modelIdEnvironmentVariable"])
        self.assertTrue(vertex["modelSelectionPacketBound"])
        self.assertEqual(["SERVICE_ACCOUNT_OAUTH"], vertex["authentication"])
        self.assertTrue(vertex["serviceAccountOnly"])
        self.assertFalse(vertex["ambientCredentialAllowed"])
        self.assertTrue(vertex["credentialFileAllowed"])
        self.assertEqual("secrets/pre-s5-vertex-service-account.json", vertex["credentialFilePath"])
        self.assertEqual("https://oauth2.googleapis.com/token", vertex["oauthTokenEndpoint"])
        self.assertEqual("https://www.googleapis.com/auth/cloud-platform", vertex["oauthScope"])
        ...
        self.assertEqual(1, vertex["maximumOAuthTokenCallsPerQuestion"])
