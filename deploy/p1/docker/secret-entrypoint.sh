...
allowed_key() {
  ...
  if [ "$key_profile" = public-demo ]; then
    case "$1" in
      DEMO_CREDENTIAL_SEPARATION_KEY|DEMO_USER_CREDENTIAL_BUNDLE|DEMO_ADMIN_CREDENTIAL_BUNDLE|KIS_*|P1_AUTOMATION_*|GOOGLE_OIDC_*) return 1 ;;
      STRONG_LLM_GRPC_SHARED_SECRET) return 0 ;;
    esac
  ...
  if [ "$key_profile" = public-full ]; then
    case "$1" in
      ...
      RETURN_INFERENCE_GRPC_SHARED_SECRET) return 0 ;;
      BROKERAGE_DB_CAPABILITY_TOKEN_SHA256|BROKERAGE_GRPC_SHARED_SECRET|GOOGLE_OIDC_CLIENT_ID|GOOGLE_OIDC_CLIENT_SECRET|GOOGLE_OIDC_ADMIN_SUBJECT_SHA256|RAG_V2_GRPC_SHARED_SECRET|RAG_V2_QUERY_DATABASE_DSN|RAG_V2_VOYAGE_QUERY_WRITER_DSN|STRONG_LLM_GRPC_SHARED_SECRET|VOYAGE_API_KEY) return 0 ;;
...
required_keys() {
  ...
  case "$key_profile" in
    ...
    public-full) printf '%s\n' 'POSTGRES_APP_PASSWORD POSTGRES_WORKER_PASSWORD POSTGRES_AUTH_PASSWORD ACTOR_CAPABILITY_SHARED_SECRET ACTOR_CAPABILITY_PUBLIC_KEY ACTOR_CAPABILITY_TLS_KEY_STORE_PASSWORD REDIS_PASSWORD JWT_SECRET JWT_ISSUER JWT_AUDIENCE LOGIN_SCOPE_HMAC_KEY PRINCIPLE_CURSOR_HMAC_KEY DECISION_IDEMPOTENCY_SCOPE_HMAC_KEY DECISION_GRPC_SHARED_SECRET BROKERAGE_IDEMPOTENCY_SCOPE_HMAC_KEY RAG_IDEMPOTENCY_SCOPE_HMAC_KEY RAG_REQUEST_FINGERPRINT_HMAC_KEY RAG_PROVIDER_USAGE_HMAC_KEY RAG_RATE_LIMIT_HMAC_KEY RAG_HISTORY_CURSOR_HMAC_KEY ASYNC_CURSOR_HMAC_KEY ASYNC_PARTITION_HMAC_KEY ASYNC_WORKER_GRPC_SHARED_SECRET STRONG_LLM_GRPC_SHARED_SECRET BROKERAGE_DB_CAPABILITY_TOKEN_SHA256 BROKERAGE_GRPC_SHARED_SECRET GOOGLE_OIDC_CLIENT_ID GOOGLE_OIDC_CLIENT_SECRET GOOGLE_OIDC_ADMIN_SUBJECT_SHA256 KIS_MOCK_ORDER_REFERENCE_KEY RAG_V2_GRPC_SHARED_SECRET RAG_V2_QUERY_DATABASE_DSN RAG_V2_VOYAGE_QUERY_WRITER_DSN VOYAGE_API_KEY RETURN_INFERENCE_GRPC_SHARED_SECRET P1_AUTOMATION_DATABASE_DSN AUTOMATION_RUNTIME_SHARED_SECRET' ;;
