...
| 항목 | 내부 collector 계약 |
|---|---|
| availability | 내부 collector/CLI와 단일 endpoint no-publish probe는 offline 구현 완료. KRX1~5/KRX8은 실패 evidence, KRX6/7/9는 TTL 만료·provider `0` evidence다. KRX8은 첫 NOW endpoint `read_timeout`, physical `1`, Redis `5→6`이고 KOSDAQ/retry/online artifact는 `0`이다. 모든 실패·만료 packet은 재사용하거나 성공 회계에 합산하지 않음 |
...
| KRX10 execution | 단일 approval packet 아래 KOSPI probe `1`→KOSDAQ probe `1`→full refresh `2` 순서다. 각 프로세스 Redis cap은 `1/1/2`; 합계 `4`는 executor가 packet 순서와 stop rule로 강제하며 app/Redis의 approval별 hard cap은 아니다. 각 성공 뒤 5분 안에 다음 단계로 이동하고 하나라도 실패하면 잔여 호출·retry·publish는 `0` |
...
| S1.3K (offline 구현 완료·live 성공 대기) | KRX private transport는 exact HTTPS origin과 `stk_bydd_trd`/`ksq_bydd_trd` GET만 허용하고 `AUTH_KEY`를 send 직전에만 부착한다. caller origin/path/auth/proxy/CA/transport override, ambient proxy/`.netrc`, redirect, response echo를 거부한다. byte/depth/list/text/row/numeric/date 상한과 exact `BAS_DD`, 6자리 종목코드, nonnegative int64, duplicate 금지를 검증한다. 단일 probe도 같은 private transport·strict parser를 사용하고 파일을 쓰지 않는다. 공식 hard cap보다 낮은 Redis rolling-24h 9,000을 유지하며 probe/full 각 프로세스 cap은 `1/1/2`, retry `0`이다. KRX1~5/KRX8 실패와 KRX6/7/9 만료 evidence는 성공 회계와 분리한다. KRX8 이후 timeout은 probe `120/130초`, full `120/260초`로 보강했다. raw provider 정보는 저장하지 않으며 최종 두 시장 성공 뒤에만 ignored manifest/report를 생성한다. public API·DB·S1.6 calendar는 변경하지 않는다 |
