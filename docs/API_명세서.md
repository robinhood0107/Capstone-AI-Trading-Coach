...
PostgreSQL role bootstrap은 `flyway`에 `log_parameter_max_length=0`과 `log_parameter_max_length_on_error=0`을 고정한다. rotation은 row lock이나 credential bind 전에 현재 세션의 두 effective setting을 직접 읽고 둘 중 하나라도 `0`이 아니면 mutation과 audit 없이 fail-closed한다. 따라서 운영상 SQL 문장 로깅(`log_statement`)은 유지할 수 있지만 BCrypt hash·reuse tag·bundle MAC bind 값은 일반 로그와 오류 로그 모두에서 생략되어야 한다. 배포 검증은 rotation과 같은 role의 새 세션에서 두 setting이 정확히 `0`인지 확인해야 하며, 확장 로거·managed-service 수집기·로그 reader ACL·retention은 이 애플리케이션 경계 밖의 별도 운영 보안 gate로 관리한다.
