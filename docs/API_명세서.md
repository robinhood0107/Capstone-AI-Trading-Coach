...
#### 13.5.A S1.3K KRX universe internal collector (offline 구현 완료·live 성공 대기)
...
통과했다. 운영 계정의 31개 서비스 entitlement는 승인됐지만 runtime allowlist는 NOW 두 개로
고정하며, 신규 KRX packet-bound 승인을 확인한 뒤에만 별도로 실행한다. 기존 실제 CSV의 `kis-universe-refresh`는 명시적
...
| 항목 | 내부 collector 계약 |
|---|---|
| availability | 내부 collector/CLI는 offline 구현 완료. KRX1~5는 Redis `0→5`의 실패 evidence다. KRX6/7은 승인 도착 전 TTL 만료로 provider `0`이다. KRX8은 첫 NOW endpoint의 `read_timeout`, physical `1`, Redis `5→6`이고 KOSDAQ/retry/online artifact는 `0`이다. KRX1~5/KRX8 실패와 KRX6/7 만료 packet은 재사용하거나 성공 회계에 합산하지 않음 |
...
| timeout/diagnostic | connect `2초`, read `30초`, write `2초`, pool `1초` I/O 상한과 두 endpoint `70초` shared logical budget, retry `0`으로 고정한다. HTTPX read timeout은 다음 response data chunk까지의 inactivity 상한이며, 5초·8초 경계 실패를 근거로 full-market headroom을 늘렸다. logical budget은 blocking DNS를 강제 취소하는 wall-clock watchdog이 아니다. credential-bearing transport 예외의 문자열·request·cause는 버리고 allowlisted stable code만 출력한다. HTTP 200 validation 실패의 외부 code는 `invalid_response`로 유지하고 media/body/JSON/root/envelope/row의 typed stage·leaf, request ordinal, NOW service, HTTP status, content/body/size/UTF-8/BOM 고정 분류, bounded count와 공식 15필드명만 suffix로 허용한다. raw body/header/URL/provider message/미확인 key·value는 금지함 |
...
| S1.3K (offline 구현 완료·live 성공 대기) | KRX private transport는 개발명세서로 확인한 exact HTTPS origin과 `stk_bydd_trd`/`ksq_bydd_trd` GET만 허용하고 `AUTH_KEY`를 send 직전에만 부착한다. `trust_env=false`, redirect=false, TLS verification=true를 고정하고 caller origin/path/auth/proxy/CA/transport override, ambient proxy/`.netrc`, response echo를 거부한다. byte/depth/list/text/row/numeric/date 상한과 exact `BAS_DD`, 6자리 종목코드, nonnegative int64, duplicate 금지를 검증한다. `-`/`0`은 0으로 정규화해 source hash에 포함하고 후보에서 제외하며, 최종 30종목의 시가총액·거래대금은 양수여야 한다. 공식 hard cap은 키당 10,000/day(0시~24시)이고 프로젝트는 Redis 원자 rolling-24h 9,000·실행당 2를 lower-only로 예약하며 refund하지 않는다. Redis/한쪽 시장/일자 검증 실패는 outbound 또는 publish 0건으로 fail-closed한다. KRX1~5/KRX8 실패 evidence와 KRX6/7 만료 evidence는 성공 회계와 분리한다. KRX5의 HTTP 200 관측성 축약은 typed validation diagnostic으로 보강했고, KRX8의 8초 read inactivity timeout은 30초 read/70초 shared logical budget으로 보강했다. retry는 계속 `0`이다. raw provider 정보는 저장하지 않으며 ignored internal manifest/report만 생성한다. public API·DB·S1.6 calendar는 변경하지 않는다 |
...
| S1.3K catalog/parser/ranking (offline 통과) | 공식 7분류·31 API ID가 NOW 2/NEXT 7/LATER 13/EXCLUDE 9로 정확히 분류되고 NOW 외 ID는 client/provider 0건임을 검증한다. official-shape sanitized fixture에서 exact date/market/code, digit-only nonnegative int64, `-` 정규화, duplicate/empty/oversize/malformed를 검증하고 KOSPI+KOSDAQ 합성 top 30의 정렬·hash 결정성을 확인한다. missing/multiple/unsupported media type, malformed/BOM/invalid UTF-8/duplicate-key JSON, non-object root, envelope/container/row/field/date/market/symbol/name/numeric/duplicate leaf도 고정함 |
| S1.3K transport/quota/storage (offline 통과) | final send에서만 AUTH_KEY 부착·종료 후 제거, fixed-origin/GET/JSON/TLS 1.2+, redirect·ambient/caller override 거부, credential/provider echo 비노출과 공식 5,000행 credential scan 경계를 검증한다. Redis 장애 outbound 0, credential 실패 시 reservation `+1`/physical `0`, quota wait 재예약·deadline 차단, 2 reservation/2 handoff, 3번째 차단, no-refund와 retry 0을 확인한다. 기본 timeout `2/30/2/1초`·shared logical `70초`, lower-only override와 상한 초과/boolean 거부, 첫 endpoint가 25초를 소비해도 두 요청 모두 read 30초를 확보함을 검증한다. 첫 endpoint read/typed validation 실패는 physical `1`/둘째 send `0`, 둘째 endpoint 실패는 ordinal `2`/service `ksq_bydd_trd`/physical `2`를 유지한다. diagnostic subclass·`str` subclass·unknown field injection은 출력 없이 거부한다. constructor/client/response cleanup 실패 비노출·resource cleanup, 순수 collector, 두 시장 성공 뒤 report→manifest 순서의 0600 fsync+atomic replace, approved ignored root containment, 서로 다른 target, 새 output directory 0700, 임시파일 정리, symlink/hardlink/path escape/raw 저장 금지와 provider 이름 Markdown escaping을 검증함 |
...
| offline 구현 완료·live 성공 대기(내부 S1.3K, public API 아님) | KRX `stk_bydd_trd`+`ksq_bydd_trd` universe refresh CLI/batch. 31개 entitlement와 NOW 2개 runtime 활성화는 분리 |
