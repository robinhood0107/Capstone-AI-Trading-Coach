...
| S6.1~S6.5 금융공학 | `IMPLEMENTED_DRAFT` | local atomic commits, V77 | 0 | offline kernel·교육 REST·manual stored batch 구현; UI consumer와 scheduler는 범위 밖 |
| S6.6 research replay | `IMPLEMENTED_DRAFT` | local code/schema/tests | 0 | real PIT candidate가 없어 `DATASET_UNAVAILABLE`, performance claim 0 |
| S6.7 P1 overlay | `IMPLEMENTED_DRAFT` | local V78 storage/reader/overlay tests | 0 | code complete, production threshold 부재로 `OFF/UNAVAILABLE`; `ENFORCED`는 `NO_GO` |
