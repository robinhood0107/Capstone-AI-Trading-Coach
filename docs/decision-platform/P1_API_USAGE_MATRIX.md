...
기준 파일은 `contracts/openapi/openapi.json`입니다. 현재 HTTP 작업은 정확히 61개이고, 아래 표에서
각 작업을 한 번씩만 분류했습니다.
...
| 42 | GET | `/api/v2/automation/status` | `Team A 필수` | 실제 control·policy·blocker·포지션 수 |
| 43 | PUT | `/api/v2/automation/policy` | `Team A 필수` | 예산·손절·익절 CAS 저장 |
| 44 | POST | `/api/v2/automation/arm` | `Team A 필수` | 현재 risk-balance blocker 409 검증 |
| 45 | GET | `/api/v2/automation/runs` | `Team A 필수` | 가변수량·체결·exit reason 실행 이력 |
| 46 | GET | `/api/v2/automation/positions` | `Team A 필수` | active bot-owned 포지션 최대 5개 |
| 47 | PATCH | `/api/v1/journals/{journalId}` | `선택 기능` | CAS 기반 학습일지 전체 교체 |
| 48 | DELETE | `/api/v1/journals/{journalId}` | `선택 기능` | CAS 기반 학습일지 soft delete |
| 49 | GET | `/api/v1/artifacts/ingest-status` | `운영자 전용` | 결과 파일 적재 상태 |
| 50 | GET | `/api/v1/async-jobs` | `운영자 전용` | 내부 작업 목록 |
| 51 | GET | `/api/v1/async-jobs/{jobId}` | `운영자 전용` | 내부 작업 상세 |
| 52 | POST | `/api/v1/brokerage/orders/{orderId}/reconcile` | `운영자 전용` | 주문 대사 |
| 53 | GET | `/api/v1/decisions/{decisionId}/audit` | `운영자 전용` | 주문 판단 감사 |
| 54 | GET | `/api/v1/stream-metrics` | `운영자 전용` | 내부 처리 지표 |
| 55 | GET | `/error` | `제품 기능 아님` | Spring 오류 처리 |
| 56 | POST | `/error` | `제품 기능 아님` | Spring 오류 처리 |
| 57 | PUT | `/error` | `제품 기능 아님` | Spring 오류 처리 |
| 58 | DELETE | `/error` | `제품 기능 아님` | Spring 오류 처리 |
| 59 | PATCH | `/error` | `제품 기능 아님` | Spring 오류 처리 |
| 60 | HEAD | `/error` | `제품 기능 아님` | Spring 오류 처리 |
| 61 | OPTIONS | `/error` | `제품 기능 아님` | Spring 오류 처리 |
...
OpenAPI 61개에 없습니다. Team A/B가 임의로 endpoint를 만들지 않습니다. 필요한 화면과 동작을
