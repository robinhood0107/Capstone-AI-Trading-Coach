...
4. LSTM/rule signal formula, confidence, XKRX session
...
9. V88 한 transaction의 Signal/Dashboard/Ingest projection
...
- `REAL_TEAM_B`, `modelQuality=PASS`, `mockRuntimeEligible=true`를 모두 만족한 검증 bundle만 production
  Signal pointer 후보가 된다. 이 문서의 synthetic 실측은 그 hard gate를 대체하지 않는다.
