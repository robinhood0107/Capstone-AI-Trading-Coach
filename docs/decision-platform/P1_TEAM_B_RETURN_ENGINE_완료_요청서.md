...
이번에 제가 V91로 예산, 가변 수량, 손절·익절을 붙였고 그 뒤 V108까지 이어 왔습니다. 이건 전부 Decision Platform 쪽 책임이고 Team B가 뭔가 더 만들어 주셔야 하는 부분이 아닙니다.
...
그 사이에 제 쪽에서 꽤 큰 변경이 하나 있었습니다. 생성형 LLM의 판단이 매수 결정에 실제로 닿도록
경로를 열었습니다. 후보 선정 앞에 상태를 하나 넣어서, 모델이 후보별 점수와 거부 여부, 전체
확신도를 내면 엔진이 그것으로 순위를 다시 세우고 수량을 줄입니다.

**그래도 Team B가 주셔야 하는 것은 하나도 달라지지 않았습니다.** 이유는 그 판단이 **여러분이 만든
후보 집합 안에서만** 움직이기 때문입니다. AI는 그 집합의 순서를 바꾸고, 일부를 막고, 수량을 줄일
수 있을 뿐입니다. 집합 밖의 종목을 넣거나 수량을 늘리는 것은 코드와 DB 제약이 함께 막습니다.
그러니 여러분 쪽 계약은 예전 그대로입니다.

| 항목 | 이전 | 지금 |
|---|---|---|
| exact-31 Rule+LSTM 신호 | 필요 | **동일하게 필요** |
| exact-10 artifact와 manifest v2 | 필요 | **동일하게 필요** |
| 두 실행 byte identity | 필요 | **동일하게 필요** |
| 일별 inference 재실행 | 필요 | **동일하게 필요** |
| 신호 schema에 새 필드 | 없음 | **여전히 없음** — AI 판단용 필드도 추가하지 말아 주세요 |
| 확신도·점수·순위를 여러분이 계산 | 아님 | **여전히 아님** — 그건 제 쪽에서 합니다 |

한 가지만 덧붙이면, AI가 답하지 못하거나 provider가 붙어 있지 않아도 자동운용은 그대로 돕니다.
그때는 판단에 "AI 미참여"를 남기고 여러분 신호와 기존 규칙만으로 진행합니다. 그러니 여러분 쪽
산출물이 AI 설정에 따라 다르게 해석되는 일은 없습니다.
