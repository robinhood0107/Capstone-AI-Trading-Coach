...
going-concern distress 이벤트는 endpoint identity만으로 이벤트가 성립한다(발생 자체가 신호). 반면 감사의견은 `adt_opinion` 같은 구조화 필드 조건이 있어야 점수화한다. 두 경우 모두 `report_nm` 문자열은 점수 근거가 아니다. distress endpoint는 이벤트별 날짜 필드명이 제각각이라, 구조 규약인 접수번호(`rcept_no`) 앞 8자리로 접수일자를 복원해 최근 30일 window를 판정한다.
