...
사용자가 처음부터 끝까지 쓰기에는 다음 부분이 아직 빠져 있습니다.

1. **화면 완성도:** 9개 화면의 정보 구조와 스타일이 최종 제품 수준으로 통일되지 않았습니다.
2. **원칙·안전장치:** 기존 원칙 수정은 되지만, 원칙이 없을 때 생성하는 흐름과 Kill Switch 변경 UI가 없습니다.
3. **주문 흐름:** 현재 주문 검토는 `decisionId` 조회 중심입니다. 주문 후보 입력, Risk 평가 요청,
   KIS 모의주문 제출, 주문 상태·취소·체결 조회까지 이어지는 흐름이 필요합니다.
4. **Automation V3:** 현재 화면은 V2 중심입니다. V3 정책, AI ON/OFF·thinking level, run 근거 상세,
   position의 ATR·trailing stop·보유 만기와 청산 이유 표시가 필요합니다.
5. **RAG·학습일지:** RAG 질문·인용은 되지만 피드백, Decision·주문·run 연결 저장, 학습일지 목록 UI가 없습니다.

위 5개를 마치면 Team A 작업은 완료입니다. 모델 계산, backend, provider 연결은 추가로 맡지 않습니다.
...
현재 UI의 위치와 순서를 그대로 지킬 필요는 없습니다. 사용하기 좋아진다면 navigation과 메뉴 순서,
화면 안의 정보 배치, 목록·표·카드 구성, button 위치를 충분히 바꿔도 됩니다. 관련 기능을 묶거나
복잡한 영역을 나누는 것도 가능합니다. 고정해야 하는 것은 route와 API 계약, 안전 동작이지 기존
레이아웃이 아닙니다.

원하는 느낌은 화려함보다 **신뢰감 있고 읽기 쉬운 금융 서비스**입니다. 한 번의 AI prompt로 만든
카드 템플릿처럼 보이지 않고, 실제 사용 흐름을 사람이 직접 다듬은 듯한 완성도가 필요합니다.

탁성우 교수님께서 말씀하신 것처럼 **UI가 조잡해 보이면 안 됩니다.** 기능을 한 화면에 단순히
늘어놓거나 서로 다른 간격·색·button을 섞지 말고, 각 화면의 핵심 목적과 첫 번째 행동이 바로
보이도록 정리해 주세요.
