...
로컬 `한국투자증권_오픈API_전체문서_20260707_030000.xlsx`의 `API 목록` 기준으로 P2에서 사용할 기능군은 다음으로 고정한다. 전체 catalog는 338개 API(REST 278개, WebSocket 60개)이며, 이 XLSX는 KIS 공식 배포 자료로서 모의 지원 경계의 단일 진실 소스다. 모의 지원 여부는 `모의 TR_ID`, `모의 Domain`, 샘플 코드를 함께 확인한다. 정확한 endpoint, KIS 요청 식별자 매핑, 지원 경계는 `선물옵션_모의주문_확장_시나리오_API_명세서.md`에만 둔다.
...
4. 모든 주문에는 `decisionId`, `marginSnapshotId`, `orderIntent`, `riskDecision`, `kisTrId`, `requestId`, idempotency key의 hash, sanitized provider receipt/hash를 남긴다. idempotency key 원문과 provider raw 응답은 남기지 않는다.
...
P2 async event payload도 P1과 동일하게 reference-only 원칙을 따른다. raw KIS 응답, 계좌 token, API key, provider 계좌번호, 개인정보, 원문 뉴스는 싣지 않고, `marginSnapshotId`, `decisionId`, `orderId`, `positionSnapshotId`, allowlisted normalized field, sanitized receipt/hash만 전달한다.
...
8. 서버가 subject/account/orderIntent/decision/marginSnapshot/action hash에 묶인 짧은 TTL의 1차 확인 receipt를 발급하고 사용자가 확인한다.
9. 1차 receipt 검증 후 서버가 별도 nonce의 2차 제출 receipt를 발급하고 사용자가 확인한다.
10. 두 receipt를 순서대로 1회 소비하고 idempotency key hash와 audit log를 기록한다. boolean 두 개를 한 요청에 보내는 방식은 2단계 확인으로 인정하지 않는다.
...
| 만료·재사용·순서가 뒤바뀐 확인 receipt | `RISK_BLOCKED`, 주문 제출 0건 |
