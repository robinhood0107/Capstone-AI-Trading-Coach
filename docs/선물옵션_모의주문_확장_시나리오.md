...
> 구현 상태: 이 문서는 v1/P1 구현 범위가 아니라 후속 P2 확장안이다. 현재 P1은 monthly PIT
> 국내주식 top 30과 금 ETF `132030`, KIS Mock, deterministic RiskEngine, 2-step RAG,
> Dashboard 흐름을 우선한다. ETN은 P1 active universe가 아니다. S1.1에서는 파생 주문을
> 구현하지 않고, KIS catalog상 지원되는 읽기 전용 시세 API만 후속 planning/source card
> 후보로 분리한다.
...
이 문서는 P1 핵심 자동매매 범위인 monthly PIT 국내주식 top 30과 금 ETF `132030` 이후에
추가되는 `국내선물옵션 주간 모의 주문` 확장 설계를 고정한다. P2는 P1을 대체하지 않는다.
P1의 데이터, RAG, Signal v2, 백테스트, RiskEngine, KIS Mock, Dashboard, Journal,
Live-ready gate를 그대로 사용하고, 그 상위에 국내선물옵션 Mock 주문 흐름을 추가한다.

핵심 결론은 다음과 같다.

1. P1 핵심 주문은 monthly PIT 국내주식 top 30과 국내 금 ETF `132030`이다.
...
|---|---|---|
| 데이터 수집 | KIS 가격·수급, OpenDART 공시·재무, ECOS 거시지표, fixture-first GDELT aggregate. Naver active 뉴스 source는 퇴역 | 국내선물옵션 후보 생성 전 시장상태, 포트폴리오 노출, 뉴스 이벤트, 거시 국면을 판단하는 입력으로 사용. GDELT는 설명 evidence이며 직접 판단 권한 없음 |
| 가격 백필 | KIS 일봉 100건 단위 반복 백필, 3년 이상 목표, 분봉은 최근 1년 이내 보조 검증 | P2 후보의 기초 포트폴리오와 hedge 필요성을 계산하는 기준 데이터로 사용 |
| 종목군 | monthly PIT 국내주식 top 30 + 금 ETF `132030`; ETN active 0 | P2 hedge 후보는 이 P1 포트폴리오 노출을 줄이는 방향으로 생성 가능 |
| RAG/LLM | deterministic 2-step RAG, BGE/Voyage context-4 두 profile·세 policy, 승인형 Gemini conditional generation, OpenAI P1 live 0 | 파생 위험 설명용 P2 source card는 별도 corpus generation 승인을 거쳐야 하며 주문 판단은 RiskEngine만 수행 |
...
| Brokerage | 사용자 명시 선택형 INTERNAL_PAPER 비상 모드(자동 전환 금지), KIS Mock 주식/ETF `132030` 주문, fail-closed 처리 | P2는 `KIS_DERIVATIVE_MOCK`을 별도 gateway로 연동하되, P1 주문 흐름과 idempotency/audit 규칙을 따른다 |
