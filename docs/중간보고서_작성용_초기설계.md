...
> 상태 주의: 이 문서는 2026-06-23 중간보고서 작성을 위해 만든 역사적 초기설계 snapshot이다. 아래의 “현재 상태”, 목표, 기술 구성은 당시 작성 초안이며 현재 구현 상태나 S1.3 이후 세션 배정의 기준이 아니다. 현재 계약·세션 상태·보안 게이트는 `최종_프로젝트_명세서.md`와 `API_명세서.md`를 우선한다.
...
> 본 프로젝트는 국내주식과 금 ETF/ETN을 대상으로 뉴스감성, LSTM, LightGBM, 백테스트, KIS Mock 주문을 결합한 수익률 중심 자동매매 시스템에 투자 원칙 엔진, RAG 금융가이드, RiskEngine을 추가하여 투자 경험이 적은 대학생이 자신의 투자전략을 학습·검증·운영할 수 있도록 돕는 AI 트레이딩 코치이다.
...
> 본 과제의 최종 목표는 국내주식과 금 ETF/ETN을 대상으로 뉴스감성, 시계열 예측, 백테스트, 투자 원칙 검증, KIS Mock 주문을 통합한 AI 자동매매 시스템을 구현하고, 사용자 테스트를 통해 학습효과·안전성·수익률/리스크·사용성을 검증하는 것이다.

### 3.2 세부 목표

| 목표 | 내용 |
|---|---|
| 데이터 기반 전략 생성 | KIS 가격 데이터, OpenDART 공시/재무, ECOS 거시지표, Naver 뉴스 metadata를 활용하고 GDELT는 optional secondary enrichment로만 검토 |
...
| 데이터 | 뉴스, 주가, KIS 중심 | KIS + OpenDART + ECOS + Naver API, optional GDELT |
...
> 시스템은 크게 Decision Platform, Return Engine, Experience Dashboard로 구성된다. Decision Platform은 LightGBM, 투자 원칙 엔진, RAG 금융가이드, RiskEngine, KIS Mock/Live-ready 주문 판단을 담당하고, 내부 비동기 이벤트 파이프라인으로 artifact ingest와 RAG indexing, model evaluation을 분리한다. Return Engine은 LSTM, 규칙 baseline, 백테스트 엔진과 artifact export를 담당한다. Experience Dashboard는 사용자가 원칙 설정, RAG 질의, 모델 평가, 백테스트 리포트, 주문 검토, 학습일지를 수행하는 화면을 제공한다.

### 5.2 시스템 구성 표

| 구성요소 | 역할 | 주요 기술 |
|---|---|---|
| Decision Platform | LightGBM, 투자 원칙, RAG, RiskEngine, 주문 판단, 내부 비동기 이벤트 처리 | JDK 25 LTS, Kotlin 2.4, Spring Boot 4.1, Gradle 9.5, Python FastAPI, LightGBM, PostgreSQL+pgvector, Redis, Kafka KRaft |
...
| Data Layer | 가격/공시/뉴스/거시 데이터 | KIS, OpenDART, ECOS, Naver Search API, optional GDELT |
...
flowchart LR
  ...
  K["KIS/OpenDART/ECOS/Naver + optional GDELT"] --> R
  K --> P
...
| 3 | 데이터 수집 | KIS, OpenDART, ECOS, Naver metadata를 수집하고 GDELT는 optional enrichment로만 검토한다 |
...
| ECOS | 금리, 환율 등 거시지표 |
| Naver Search API | 국내뉴스 제목/요약/메타데이터 |
| GDELT | optional 외신/글로벌 이벤트 secondary enrichment. S1.3 blocker 아님 |
| 공식자료/API/source card | RAG 근거 자료 |

작성 문장:

> 뉴스 본문 크롤링은 v1 핵심에서 제외하고, API 기반 뉴스 메타데이터와 공식자료·공시 본문을 중심으로 구성한다. 뉴스 원문 전체는 RAG에 넣지 않고, Return Engine이 생성한 뉴스감성 요약 artifact를 RAG가 설명하는 방식으로 연결한다. 이를 통해 최신성은 활용하되 노이즈, 중복 기사, 저작권·약관 리스크를 줄인다.
...
| LightGBM | 기술지표, 뉴스감성, 공시, 거시지표를 결합하는 tabular feature baseline |
| HMM | 미래 가격 직접예측이 아니라 고변동/하락 등 시장국면 리스크 필터 |
| PatchTST/TFT | 관련 연구로만 언급(비교 실험 없음) |
| QLoRA/파인튜닝 | 후순위 연구 |

작성 문장:

> 모델은 LSTM만 사용하지 않고 LightGBM과 규칙 기반 baseline을 함께 두어 모델 성능을 비교한다. 팀원 B는 LSTM과 규칙 baseline, 팀원 1은 LightGBM을 맡아 ML/백테스트 경험을 균형 있게 나누고, 팀원 A는 모델 평가/백테스트 리포팅 UI를 맡아 결과 해석 경험을 확보한다. 직접 구현하는 모델은 금융 LLM이 아니라 수익률 예측과 전략 검증 모델이며, 금융 설명과 질의응답은 Strong LLM과 전용 RAG를 결합해 처리한다.

작성 문장:

> LSTM은 기존 착수보고서의 정체성을 유지하는 시계열 모델로 두되 단독 판단 근거로 사용하지 않는다. LSTM/규칙 baseline/LightGBM이 만든 신호는 RiskEngine에서 원칙 위반, HMM 시장국면, 변동성, VaR/CVaR, 뉴스감성 충돌 여부와 함께 검증한 뒤 ALLOW/WARN/HOLD/BLOCK으로 변환한다.
...
| 영역 | 후보 | 중간보고서 표현 |
|---|---|---|
| RAG 검색 | `BAAI/bge-m3`, `BGE-m3-ko`, `multilingual-e5` | 한국어/영어 공식자료 검색 품질을 비교한 뒤 채택 |
...
| 외부 활용 | Strong LLM, bge-m3 계열 embedding, FinBERT 후보 | 검증된 공개 모델을 부품으로 활용 |
| 제외 | 뉴스 원문 전체 RAG, Finance-RAG 완제품, HF trading bot | 범위 과대화와 신뢰도 문제를 방지 |
| 평가 | 50문항 RAG 평가셋, citationCoverage, Recall@5, directAdviceBlockRate | 출처 기반 설명 품질을 수치화 |
...
| 담당 | 역할명 | 핵심 책임 | 산출물 |
|---|---|---|---|
| 팀원 1 | LightGBM + Decision Platform + RAG/Risk | LightGBM, 원칙 엔진, RAG 전체 품질 책임, RiskEngine, HMM/위험지표, KIS Mock/Live-ready, 금융공학 기준 검증 | `lightgbm_signal.json`, API, RAG, RiskEngine, 주문 판단, 금융공학 분석 리포트 |
...
| 3 | 데이터 pipeline | KIS/OpenDART/ECOS/Naver + optional GDELT → feature → 모델 |
...
| 실현가능성/완성도 35 | KIS Mock, OpenDART, Naver API, Docker Compose, OCI, CI |
...
| 뉴스 데이터 | 설계 반영 | Decision Platform의 Naver metadata snapshot을 소비하고 GDELT는 optional sample | 팀원 B |
| 뉴스감성 artifact | 설계 확정 | `news_sentiment_summary` sample export | 팀원 B |
...
| [ ] | OpenDART, ECOS, Naver API가 필수 데이터 원천으로 포함되고 GDELT가 optional로 구분됐는가 |
