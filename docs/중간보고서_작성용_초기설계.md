...
| 교차시장 즉시 차단·실시간 feed | 계획은 `PLAN_FEASIBILITY=GO`, S4.8A 계약은 잠겼지만 runtime은 `NOT_IMPLEMENTED / PLANNED`; P1은 fixture/EOD 기반 `WARN_ONLY` |
...
> 교차시장·애널리스트 부분은 S4.8A 계약만 잠겼고 runtime은 `NOT_IMPLEMENTED / PLANNED`인 최종 목표 구조다.
