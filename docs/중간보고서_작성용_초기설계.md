...
| 교차시장 즉시 차단·실시간 feed | 현재 `PLAN_FEASIBILITY=GO_WITH_EXTERNAL_HARD_GATES`; S6.6 research code와 S6.7 P1 `WARN_ONLY` code/storage는 구현됐지만 real dataset/threshold가 없어 운영은 `OFF/UNAVAILABLE`; 실시간 feed와 차단은 미승인 |
...
> 2026-08-21 갱신: S6.6 research code와 S6.7 v2 storage/reader/P1 overlay code는 구현됐다.
> real PIT dataset과 production threshold가 없어 operational activation은 `OFF/UNAVAILABLE`이며,
> `ENFORCED` HOLD/BLOCK은 계속 `NO_GO`다.
