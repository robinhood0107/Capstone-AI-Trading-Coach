...
| 목표 | 내용 |
|---|---|
| 데이터 기반 전략 생성 | KIS 가격 데이터, OpenDART 공시/재무, ECOS 거시지표, Naver/GDELT 뉴스 데이터를 활용 |
...
| 데이터 | 뉴스, 주가, KIS 중심 | KIS + OpenDART + ECOS + Naver API + GDELT |
...
| Data Layer | 가격/공시/뉴스/거시 데이터 | KIS, OpenDART, ECOS, Naver Search API, GDELT |
...
flowchart LR
  ...
  K["KIS/OpenDART/ECOS/Naver/GDELT"] --> R
  K --> P
...
| 3 | 데이터 수집 | KIS, OpenDART, ECOS, Naver/GDELT에서 데이터를 수집한다 |
...
| KIS Live 8종 | Mock 5종 + Price Tolerance, Cooldown/Repeated Loss, Rate Limit/Reconciliation, Consent/Audit Log |
...
| 3 | 데이터 pipeline | KIS/OpenDART/ECOS/Naver/GDELT → feature → 모델 |
...
| 뉴스 데이터 | 설계 반영 | Naver/GDELT sample 수집 | 팀원 B |
...
| [ ] | OpenDART, ECOS, Naver API, GDELT가 데이터 원천에 포함됐는가 |
