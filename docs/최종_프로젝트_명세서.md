...
| S2.3 Decision runtime | ACTIVE Principle은 `principle_id + user_id + current immutable version` 한 조회로 pin하고 missing/cross-owner/inactive를 동일 404로 숨긴다. Decision/trace/artifact/audit/outbox/idempotency는 owner scope와 exact append-only privilege로 원자 저장한다. S1.1 instrument catalog는 append-only/versioned/latest/bounded reader와 전용 writer로만 연결하고 false/0 fallback을 금지한다. invalid selector는 400, expected source unavailable은 persisted 200 HOLD, invariant/serialization/commit 실패는 5xx다. OpenAPI는 승인된 route/component allowlist와 S2.3 catalog hash를 검증한다 |
