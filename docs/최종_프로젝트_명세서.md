...
V11 ledger는 `one Decision = one order`, HMAC idempotency replay/conflict, owner RLS projection,
append-only submit/cancel order event와 sanitized outbox를 강제한다. balance/buyable은 S2.3
stored-source의 opaque account scope projection만 읽고 raw 계좌번호를 공개하지 않는다. Spring/Python
Brokerage gRPC boundary는 shared-secret·circuit-breaker wiring까지 두되 기본 disabled다. 만료
