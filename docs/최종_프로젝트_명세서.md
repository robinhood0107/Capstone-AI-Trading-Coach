...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·실패 KRX1~3 분리·live 성공 대기(NOW 2개 endpoint만) |
...
offline fixture/mock 기반 collector 검증 뒤 KRX1~KRX3 live packet은 모두 실패 evidence로
종료했다. KRX1은 credential 검증 전에 Redis `0→1`만 예약해 physical `0`, KRX2와 KRX3는 각각
첫 `stk_bydd_trd` handoff에서 인증 실패해 physical `1`, Redis `1→2`와 `2→3`을 기록했으며
...
`3bfcc6e66bcac498323d6e14e6911acbfab9a71a04b4e95d0b5395b9809bcf36`이며, 모두 ignored
operator evidence mode `0600`이다. 세 packet은 만료·소비되어 재사용하지 않고 성공 evidence와
합산하지 않는다. 현재는 31개 서비스 entitlement를 확인했지만 runtime은 NOW 두 개만 유지하며,
새 HEAD·거래일·순서·Redis 기준값·TTL에 결속한 신규 packet 승인 뒤 성공 smoke를 1회 수행해야
