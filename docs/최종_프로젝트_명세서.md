...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·실패 KRX1~5/KRX8 분리·KRX6/7 만료 미실행·timeout 보강 완료·KRX9 live 성공 대기(NOW 2개 endpoint만) |
...
### 11.1.4 S1.3K KRX OPEN API universe 자동화 (별도 PR offline 구현·검증 완료, live 성공 대기)

S1.3K는 사용자가 수동으로 받던 KRX 시가총액 상위 CSV를 Decision Platform 내부의
고정 KRX OPEN API collector로 대체하는 **S1.3의 별도 후속 트랙**이다. 기존 A4/B1
evidence를 재해석하거나 재사용하지 않는다. collector·CLI·offline fixture/mock 검증은 별도
브랜치에서 구현했다. 운영 계정은 31개 서비스의 활용 승인을 받았지만 entitlement와 runtime
활성화는 별개이며, 이 트랙은 NOW 두 서비스와 신규 KRX online 승인 packet만 허용한다. 공식 근거는 [KRX OPEN API 서비스 목록](https://openapi.krx.co.kr/contents/OPP/INFO/service/OPPINFO004.cmd),
...
- `NOW`: S1.3K 자동화 PR에 구현된 정확한 allowlist다. live 활성화에는 별도 승인이 필요하다.
...
| 실행 시각 | 완료 거래일 `D`의 다음 날 `08:10 KST` 이후 CLI/batch를 1회 호출하는 프로젝트 운영 계약. 이 시각은 KRX SLA가 아니며 이 PR은 별도 scheduler를 추가하지 않음 |
| transport timeout/진단 | connect `2초`, read `30초`, write `2초`, pool `1초`를 각 I/O hard 상한으로, 두 NOW endpoint 전체는 `70초` shared logical budget으로 둔다. HTTPX read timeout은 총 응답시간이 아니라 다음 response data chunk까지의 무응답 상한이다. KRX4의 과거 5초 경계와 KRX8의 확정 8초 `read_timeout`을 근거로 full-market 응답 headroom을 늘렸으며, blocking DNS까지 강제 중단하는 wall-clock watchdog으로 주장하지 않고 자동 retry는 계속 `0`이다. transport 예외는 문자열·request·cause를 폐기하고 stable code만 노출한다. HTTP 200 validation 실패는 외부 code `invalid_response`를 유지하되 `media_type`, `response_body`, `json_decode`, `json_limits`, `root_shape`, `envelope_shape`, `row_shape`, `row_semantics`의 allowlisted stage/leaf와 request ordinal·NOW service·HTTP status·고정 분류/count/공식 field만 추가한다. body/header/실제 미확인 key/value/provider message는 진단에 저장하거나 출력하지 않음 |
| 선정 | KOSPI+KOSDAQ canonical row를 합친 뒤 `marketCap desc -> tradingValue desc -> symbol asc`로 정렬함. 공식 `-` 또는 `0` 시가총액·거래대금은 0으로 정규화해 source hash에는 포함하되 후보에서는 제외하고, 양수인 종목 중 정확히 30개를 생성 |
...
`53bc9d4e001b839fe2692be61271f5954fc9e9703b6f94501ac782b128029d62`이다. KRX1~5와 KRX8은
소비된 실패 evidence, KRX6/7은 만료 미실행 evidence로 분리하며 모두 재사용하거나 성공 회계에
합산하지 않는다. 현재 runtime은 NOW 두 개만 유지하며 새 HEAD·거래일·순서·`2/30/2/1초` timeout·
`70초` shared logical budget·Redis 기준값 `6`·TTL에 결속한 KRX9 packet 승인 뒤 성공 smoke를
1회 수행해야 한다. 주기 scheduler는 아직 추가하지 않았다.
