...
projection만 읽고 raw 계좌번호를 공개하지 않는다. Spring/Python Brokerage gRPC boundary는
shared-secret·circuit-breaker wiring까지 두되 기본 disabled다. 만료
...
KIS/broker/live account/order provider를 물리 호출하지 않는다.
