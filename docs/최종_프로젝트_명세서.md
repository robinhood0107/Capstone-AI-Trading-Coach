...
S1.2에서 실제 지원하는 endpoint, 공시 위험 점수 등급, 의도적 제외(원본파일/XBRL/전체 재무제표/개인별 보수)와 후속 필수 축(다중회사 재무지표, DS004 지분공시, BW/EB/합병·분할)은 `docs/decision-platform/S1_2_OpenDART_공시위험점수_근거.md`를 단일 진실 소스로 둔다. OpenDART 전체 85개(DS001~DS006)를 모두 구현하지는 않는다.
...
| 항목 | S1.1 기준 |
|---|---|
| 환경 설정 | `KIS_MODE=mock\|live`, `KIS_OFFLINE=0\|1`, `KIS_RATE_LIMIT_PER_SECOND`, `REDIS_URL`, `KIS_DATA_DIR` |
...
| 항목 | 기준 |
|---|---|
| Secret | API key/secret/token은 환경변수로만 주입. repo에는 `.env.example`(키 이름만)만 커밋. 시크릿 스캐너(gitleaks 등)를 CI에 포함 |
