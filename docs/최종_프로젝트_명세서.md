...
| 계층 | 역할 | 적용 방식 |
|---|---|---|
| Strong LLM | 금융 개념 설명, 근거 요약, 보고서/학습일지 문장화 | OpenAI를 기본 provider로 사용하되 assistant/Gemini 등 provider adapter를 유지 |
...
| 최대 금 ETF/ETN 비중 | `gold_etf_etn_weight <= 0..1` |
...
| 신호 stale 방어 | optional model evidence이면 WARN+ABSTAIN, 명시적으로 required인 input이면 HOLD. missing/stale 자체를 threshold BLOCK으로 표현하지 않음 |
