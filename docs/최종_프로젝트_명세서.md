...
### 8.3.1 Decision 유효시간 (S2.3 계획)

S2.3에서 persisted 주문 승인(decision)을 발급하면 기본 10분 동안만 유효하게 한다(`validUntil`).
만료된 decision으로는 주문할 수 없고 재평가가 필요하다. Kill Switch 활성화, 원칙 새 버전 저장,
데이터 신선도 invalidation 시 미사용 decision은 즉시 무효화하며 decision 1건은 주문 1건에만
사용한다. 이 규칙은 "과거 가격/신호로 받은 승인으로 현재 주문을 내는" 시간차 문제를 차단한다.
현재 S2.2는 offline evaluator/result 계약까지만 구현하므로 `decisionId` 발급·저장·조회와
OpenAPI route가 아직 제공된다는 뜻이 아니다. 계획 API 계약은 API 명세서 5장을 따른다.
...
`JdbcPrincipleSnapshotAdapter`만 유일한 production adapter 예외다. S2.3이 이 read port와
나머지 source port의 owner-scoped runtime/persistence/Decision API와 OpenAPI를 연결하고, S3가
KIS Mock/INTERNAL_PAPER balance·order source adapter를 연결한다.
...
금지한다. S2.3 계획 wire에서 invalid selector는 decision 없는 `400 VALIDATION_ERROR`,
...
그 조회 자체는 S2.2의 내부 `JdbcPrincipleSnapshotAdapter`가 한 SQL로 구현한다. S2.2는 이를
controller/bean/runtime route에 연결하지 않으며 Brokerage, risk, disclosure, signal production
adapter나 production fake/fallback을 추가하지 않는다.
...
`portfolio_balance_observations`/`portfolio_position_observations`, 기존 INTERNAL_PAPER ledger와
V6 disclosure observation이 canonical source다. S2.3 `decision_app`은 SELECT만 가지며 V9는
production source row나 0/빈 fake를 seed하지 않는다. row 부재·stale·incomplete·future timestamp는
typed unavailable로 persisted 200 HOLD가 된다. 따라서 S1.1/S3 producer 배포 전 환경은 안전하게
HOLD-only일 수 있고, provider producer 구현과 live/mock order 실행 권한은 S2.3 범위가 아니다.
...
| S2.3 Decision runtime (계획) | ACTIVE Principle은 `principle_id + user_id + current immutable version` 한 조회로 pin하고 missing/cross-owner/inactive를 동일 404로 숨긴다. Decision resource/persistence는 owner scope, exact runtime privilege, append-only audit와 bounded idempotency를 적용한다. invalid portfolio selector는 400, unavailable selected context의 completed evaluation은 200 HOLD이며 invariant/serialization 실패는 5xx다. OpenAPI path가 승인·추가되기 전에는 호출 불가다 |
...
| 항목 | 목표 |
|---|---|
| 주문 평가(decision) 응답 | S2.3 route 구현 후 측정할 목표 p95 < 1초. S2.2 offline evaluator의 구현 완료 수치가 아님 |
| RAG 답변 | p95 < 8초 |
