...
| S1.2~S1.4 analysis data | S1.2 OpenDART 공시, S1.3 ECOS 거시/Naver 뉴스 메타, S1.4 수익률·리스크 지표 계산 모듈, S1.2+ Market Calendar/Event Aggregator(11.1.2, 계획) | KIS 외 데이터와 순수 계산 위주로 순차 구현. KIS 순위/업종/지수 시세는 필요 시 후속 planning으로만 검토 |
...
### 11.1.2 Market Calendar/Event Aggregator (계획, S1.2+)
...
| 단계 | S1.1은 로컬 `exchange_calendars` XKRX 판정만 사용(비거래일 KIS 호출 회피). 다중 소스 수집·정규화는 S1.2+, 내부 API 공개는 S1.4+ 계획 |
...
| 로컬 캘린더 | 비거래일 KIS 호출 회피용 로컬 `exchange_calendars` XKRX 판정만 사용. 다중 소스 캘린더/이벤트 집계는 S1.1 범위가 아니며 11.1.2(계획, S1.2+)를 따름 |
