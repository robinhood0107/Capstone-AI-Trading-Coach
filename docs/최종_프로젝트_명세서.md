...
장애·cap 소진·deadline 초과는 outbound 전에 fail-closed한다. 주문성 호출은 retry 0,
...
접수 뒤 `COMMITTED`로 원자 전환한다. commit 실패 시 pending을 recovery anchor로 남긴 채
in-memory reference로 전량취소를 retry 없이 최대 1회 보상한다.
