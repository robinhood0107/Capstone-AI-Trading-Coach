...
작성일: 2026-06-23  
기준 문서: `착수보고서.md`, `2026전기_착수보고서_외신 및 국내뉴스 감성분석 및 LSTM 기반_v0.1.docx`, `한국투자증권_오픈API_전체문서_20260504_030007.xlsx`, `LLM_trader`, `LLM_trader_docs`, `open-trading-api`
...
KIS XLSX 기준으로 문서화할 주요 API는 다음과 같다.

| 기능 | Endpoint | TR ID |
|---|---|---|
| 국내주식 현재가 | `/uapi/domestic-stock/v1/quotations/inquire-price` | `FHKST01010100` |
| 국내주식 일자별 | `/uapi/domestic-stock/v1/quotations/inquire-daily-price` | `FHKST01010400` |
| 국내주식 기간별 시세 | `/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice` | `FHKST03010100` |
| 주식 당일 분봉 | `/uapi/domestic-stock/v1/quotations/inquire-time-itemchartprice` | `FHKST03010200` |
| 주식 일별 분봉 | `/uapi/domestic-stock/v1/quotations/inquire-time-dailychartprice` | `FHKST03010230` |
| ETF/ETN 현재가 | `/uapi/etfetn/v1/quotations/inquire-price` | `FHPST02400000` |
| 국내주식 현금주문 | `/uapi/domestic-stock/v1/trading/order-cash` | 실전 `TTTC0012U/TTTC0011U`, 모의 `VTTC0012U/VTTC0011U` |
| 국내주식 정정취소 | `/uapi/domestic-stock/v1/trading/order-rvsecncl` | 실전 `TTTC0013U`, 모의 `VTTC0013U` |
| 잔고조회 | `/uapi/domestic-stock/v1/trading/inquire-balance` | 실전 `TTTC8434R`, 모의 `VTTC8434R` |
| 주문가능조회 | `/uapi/domestic-stock/v1/trading/inquire-psbl-order` | 실전 `TTTC8908R`, 모의 `VTTC8908R` |
| 체결통보 WebSocket | WebSocket | 실전 `H0STCNI0`, 모의 `H0STCNI9` |
...
| 영역 | 추천 |
|---|---|
| Backend API | Kotlin Spring Boot + Python FastAPI 병행 |
...
| Spring/Kotlin 중심 | 한국 IT 산업 친화성, 구조 안정성 | ML/RAG 연동 복잡도 증가 | Decision API에 적합 |
| Python ML Worker + Spring/Kotlin API | 구조적 안정성이 높음 | 팀 3명 기준 복잡도 증가 | 필요한 부분만 적용 |

최종 판단은 `Kotlin/Spring은 Decision API와 운영 구조`, `Python/FastAPI는 RAG/ML/KIS 실험과 worker`로 역할을 나누는 것이다.
...
| ELW/선물옵션 읽기 전용 데이터 수집(v1) | 제외 확정(2026-07-04) | 모의투자 미지원 API 다수로 시연 불가. IV 계산기는 수동 입력으로 유지, 선물옵션은 P2 확장 문서(`선물옵션_모의주문_확장_시나리오.md`)에서만 관리 |
...
| KIS 공식 근거 | 한국투자증권 모의투자 안내, 한국투자증권 수수료 안내, KIS OpenAPI XLSX, `open-trading-api` 예제 |
