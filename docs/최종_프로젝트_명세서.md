...
| 항목 | 계약 |
|---|---|
| 입력 | 완료 거래일 `D`의 exact `basDd=YYYYMMDD`. 명시한 `--as-of`는 XKRX 거래일·안전 최신일 이하여야 하고, 생략하면 로컬 XKRX calendar와 `08:10 KST` 안전 경계로 최신 가용일을 정함. provider 실패를 이전 날짜 재호출로 바꾸지 않음 |
...
| 저장 | provider raw body/header/request URL은 저장하지 않음. ignored Decision Platform root의 기존 `UniverseManifest` v1과 Markdown report에 고정 source label, 기준일, 검증된 전체 canonical row SHA-256, 선정 30개를 기록함. physical attempt는 runtime counter로만 유지하며 별도 provenance 파일은 만들지 않음 |
