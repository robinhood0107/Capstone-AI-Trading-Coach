...
> 회귀 검증을 완료했다. Approval A1·A2·A3는 실패 evidence로 분리하며, 신규 A4
...
> `0`회다. Approval B는 전체 offline gate·원격 CI green, 최신 완료 거래일 KRX universe
> audit, Naver 내부 사용·metadata 최대 30일 보존 승인이 모두 끝날 때까지 발급하지 않는다.
...
의미 승인은 B 승인 권한이 아니며, 남은 KRX·Naver 선행조건과 새 B packet을 생략하지 않는다.
