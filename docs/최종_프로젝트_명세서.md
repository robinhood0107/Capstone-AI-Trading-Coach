...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·실패 KRX1~5/KRX8/KRX10 분리·KRX6/7/9 만료 미실행·단일 endpoint probe/timeout/issue-code 경계 보강 완료·KRX11 단계형 live 성공 대기(NOW 2개 endpoint만) |
...
| 실행 시각 | 정상 운영은 완료 거래일 `D`의 다음 날 `08:10 KST` 이후 full refresh CLI/batch를 1회 호출한다. 현재 recovery는 단일 승인 아래 KOSPI probe→KOSDAQ probe→full refresh 3개 명령을 순차 실행하며 이 PR은 scheduler를 추가하지 않음 |
...
`[0-9]{6}`으로 경계를 분리했다. 다음 KRX11은 final HEAD·거래일·세 명령 순서·probe
`2/120/2/1초+130초`·full refresh `2/120/2/1초+260초`·발급 직전 Redis rolling-24h 기준값·
TTL `60분`에 결속한다.
