...
별도 registry activation과 재검증 뒤 다시 승인받아 ECOS data 2회와 legacy Naver 1-query 1회를
retry 0·`--require-complete`로 실행한다. Approval B quota는 ECOS key `+2`, Naver legacy key `+1`,
파생 합계 `3`으로 기록한다. A와 B 사이 rolling window가 만료될 수 있으므로 단일 현재 count가 아니라
두 승인 evidence를 합산해 ECOS `6` + Naver `1` = 전체 physical attempt `7`을 판정한다. quota
