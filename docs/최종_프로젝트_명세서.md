...
바꾸지 않고 sanitized event/audit를 남긴다. `CANCEL_REQUESTED -> FILLED` race는 허용하고 종료
상태 이후 관측은 Invalid다. 평균 체결가는 서버가 정수 가중 평균으로 다시 계산하며 DB 판정과
Kotlin 판정이 다르면 transaction을 fail-closed한다.
...
bounded SECURITY DEFINER 함수만 실행한다. reconcile은 최대 200개, owner fill page는 KST 최대
31일·50개와 HMAC cursor로 제한한다. MISMATCH는 자동 수정하지 않고 warning/audit로만
