...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·실패 KRX1~5 분리·typed 진단 보강 완료·KRX6 live 성공 대기(NOW 2개 endpoint만) |
...
| transport timeout/진단 | connect `2초`, read `8초`, write `2초`, pool `1초`를 각 I/O hard 상한으로, 두 NOW endpoint 전체는 `20초` logical budget으로 둔다. blocking DNS까지 강제 중단하는 wall-clock watchdog으로 주장하지 않으며 자동 retry는 계속 `0`이다. transport 예외는 문자열·request·cause를 폐기하고 stable code만 노출한다. HTTP 200 validation 실패는 외부 code `invalid_response`를 유지하되 `media_type`, `response_body`, `json_decode`, `json_limits`, `root_shape`, `envelope_shape`, `row_shape`, `row_semantics`의 allowlisted stage/leaf와 request ordinal·NOW service·HTTP status·고정 분류/count/공식 field만 추가한다. body/header/실제 미확인 key/value/provider message는 진단에 저장하거나 출력하지 않음 |
...
`d65f5b248c6dfa397be1340b3b884e68c653ad2bcad058fc1640840893844b3b`이다. 공식 성공 명세와 공식
sample 응답은 기존 strict parser가 수용했으므로 성공 계약을 느슨하게 하지 않고 typed validation
leaf만 추가했다. 다섯 packet은 만료·소비되어 재사용하지 않고 성공 evidence와 합산하지 않는다.
현재 runtime은 NOW 두 개만 유지하며 새 HEAD·거래일·순서·timeout profile·Redis 기준값 `5`·TTL에
결속한 KRX6 packet 승인 뒤 성공 smoke를 1회 수행해야 한다. 주기 scheduler는 아직 추가하지 않았다.
