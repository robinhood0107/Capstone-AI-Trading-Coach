...
| 역할 | 의미 |
|---|---|
| 자동매매 엔진 | 주식/금 ETF·ETN KIS Mock 주문, INTERNAL_PAPER fallback, 백테스트, 신호 생성, 포트폴리오 상태 추적 |
...
> 기존 착수보고서의 24시간 감시형 수익률 지향 자동매매 구조를 유지하되, 투자 경험이 적은 대학생이 자신의 투자 원칙을 세우고, RAG 금융가이드와 뉴스감성·LSTM·LightGBM·백테스트·KIS Mock을 통해 주식/금 ETF·ETN 전략을 검증하며, RiskEngine으로 위험한 주문을 경고하거나 제한하는 투자 원칙 검증형 AI 자동매매 시스템으로 고도화한다.
...
| 뉴스 본문 크롤링 리스크 | Naver Search API, GDELT, 공식자료 본문, 공시 중심으로 구성 |
...
| 실현가능성/완성도 | 35 | KIS Mock, OpenDART, Naver API, GDELT, PostgreSQL, Docker Compose 기반 v1 범위 제시 |
...
flowchart TD
  ...
  B --> C["RAG 금융가이드"]
  C --> D["데이터 수집: KIS/OpenDART/ECOS/Naver/GDELT"]
    ...
    decision-platform/       # 팀원 박종진(robinhood0107) 담당
      ...
      python-services/
        app/
          ...
          data/               # OpenDART/ECOS/KIS market/Naver/GDELT client
...
2. `modelVersion`, `schemaVersion`은 manifest에 항상 기록하고, breaking change는 새 버전 번호를 사용한다.
...
15. artifact ingest, RAG indexing, model evaluation 같은 장시간 작업은 주문 승인 판단과 분리한다. 주문 승인 판단은 Spring RiskEngine 동기 처리로 유지하고, 상세 구현은 팀원 1 상세 구현명세서에서 관리한다.
...
| 7 | Rate Limit/Reconciliation | API 호출 제한, 주문/체결/잔고 대조 |
...
1. 사용자가 고급해제를 선택한다.
2. 최소 안전장치 8종이 모두 켜져 있어야 한다.
3. 실거래 위험 안내를 확인한다.
4. 전략 요약, 손실 가능성, 주문 권한을 3단계로 동의한다.
5. 투자 원칙, 주문 상한, 종목 universe, RiskEngine 기준이 변경되면 재동의한다.
6. Live 거래 전 Mock 또는 내부 paper 결과가 기준을 통과해야 한다.
...
> 변경 반영(2026-07-08): optional 항목 추가. Market Calendar/Event Timeline은 S1.4 이후 optional dashboard panel로 제공한다. 프론트엔드 초기 범위에는 포함하지 않으며, API가 준비되면 관심종목의 휴장일, 실적 예정/발표, 배당, 분할, IPO, 주요 공시, macro release를 timeline 형태로 표시한다(계약: `API_명세서.md` 12A).
...
| GDELT | 글로벌 이벤트, 메타데이터, 요약 feature | 필수 |
| FRED | 미국 거시지표 보조 | 후순위/보조 |
| 공식 금융자료 | RAG 근거 | 필수 |

가격·수급 데이터는 KIS를 1차 source로 둔다. 일봉은 KIS 기간별 시세 API를 100건 단위로 반복 호출해 3년 이상 백필하는 것을 목표로 하고, 분봉은 KIS 제공·보관 한도상 최근 1년 이내의 보조/forward 검증 데이터로 제한한다. KRX, pykrx, FinanceDataReader 등은 KIS 결측 보정 또는 비교 검증용 보조 수급처로만 안내한다.
...
| S1.2~S1.4 analysis data | S1.2 OpenDART 공시, S1.3 ECOS 거시/Naver 뉴스 메타, S1.4 수익률·리스크 지표 계산 모듈, S1.2+ umbrella의 Market Calendar/Event Aggregator(확정 구현 S1.6, 11.1.2 계획) | KIS 외 데이터와 순수 계산 위주로 순차 구현. KIS 순위/업종/지수 시세는 필요 시 후속 planning으로만 검토 |
...
>
> 배정 정합화(2026-07-10): `S1.2+`는 상위 umbrella 표현이며 다중 소스 aggregator의 확정 구현 세션은 S1.6이다. 상세 공개 근거는 S1.2 OpenDART 공시위험점수 근거 문서를 따른다.
...
| 소스 분류 | `OFFICIAL_FREE` / `OFFICIAL_KEYED_FREE` / `FREE_TIER_AGGREGATOR` / `SCRAPE_OR_RSS_ONLY` / `PAID_OR_UNKNOWN`(기본 배제, 채택은 별도 사용자 결정) / `UNSAFE_OR_EXCLUDE`(사용 금지) |
...
| 단계 | S1.1은 로컬 `exchange_calendars` XKRX 판정만 사용(비거래일 KIS 호출 회피). 다중 소스 수집·정규화는 S1.2+ umbrella 아래 S1.6에서 구현하고, 내부 API 공개는 S1.4+ 계획 |
...
- GDELT: 글로벌 이벤트·메타데이터·요약 feature만 활용하고, 원문 전체를 런타임 RAG corpus로 넣지 않음.
...
| 항목 | S1.1 기준 |
|---|---|
| 환경 설정 | `KIS_MODE=mock\|live`, `KIS_OFFLINE=0\|1`, `KIS_RATE_LIMIT_PER_SECOND`, `KIS_DATA_DIR`, `REDIS_HOST`/`REDIS_PORT`/`REDIS_DB`; credential/password는 env-only private transport 경계에서만 소비 |
| 토큰 | `/oauth2/tokenP` access token을 Redis `kis:token`에 저장하고 만료 5분 전부터 갱신. KIS 토큰 유효기간은 1일이며 발급 후 6시간 이내 재요청 시 기존 토큰이 반환된다 |
...
| 백필 | 일봉 100건 단위 반복 호출로 3년 이상 목표. append/upsert는 idempotent하게 처리 |
| 저장 | raw 응답은 로컬 ignored 경로에만 두고, commit 가능한 fixture는 마스킹된 offline fixture만 허용 |
| 재시도 | GET market-data는 제한적으로 재시도. POST 주문성 요청은 S1.1 범위 밖이며 재시도 정책도 구현하지 않음 |
...
`KIS_MODE=live`는 S1.1에서 실전 Domain의 시장데이터를 읽을 수 있다는 의미일 뿐, 실계좌 주문 권한을 활성화한다는 뜻이 아니다. 국내휴장일조회, ETF/ETN 현재가, 주식 일별 분봉처럼 모의투자 미지원인 조회 API는 실전 Domain으로만 호출되므로 이 live read-only 경계에 속한다. Live account read-only API는 catalog와 source card에 정리할 수 있지만, 잔고·주문가능·체결조회는 S3 Brokerage 범위에서 Mock 우선으로 다룬다. 실계좌 주문·정정·취소는 live-order gate, 명시적 사용자 동의, kill switch, 주문 상한, audit/reconciliation 검증이 모두 갖춰지기 전까지 기본 OFF다.
...
| Brokerage | KIS OpenAPI Mock 중심, INTERNAL_PAPER fallback, Live-ready 비활성 설계 |
| Data | KIS, OpenDART, ECOS, Naver Search API, GDELT |
...
| API retry | 쓰기 요청은 idempotency key를 사용하고, 실패 재시도는 audit log에 남김 |
...
| 주문 중복 | `X-Idempotency-Key`, `orderIntentId`, `decisionId`, `orderId` unique 기준으로 방어 |
...
| 항목 | 기준 |
|---|---|
| Secret | provider app key/secret·운영자 API key 같은 정적 credential은 환경변수로만 주입. repo에는 `.env.example`(변수 이름만)만 커밋. 시크릿 스캐너(gitleaks 등)를 CI에 포함. provider가 발급한 단기 bearer token은 인증된 Redis TTL cache에만 두고 정적 credential과 혼용하지 않음 |
| OpenDART 인증 | 인증정보는 공개 settings·business/HTTP client가 보관하지 않는다. 고정 OpenDART HTTPS origin의 private transport가 send 순간에만 env에서 일시 로드하고 request URL을 즉시 원복한다. caller origin/인증 파라미터 주입은 전송 전에 거부하고 response·로그·예외·raw/fingerprint의 값과 민감 필드는 제거한다 |
| KIS 인증 | app key/secret과 bearer token은 공개 settings·market/business client에 두지 않는다. private token issuer가 app key/secret을 env에서 읽고 발급 bearer는 인증된 Redis TTL cache에서 가져오며, fixed-origin transport만 승인된 read-only endpoint/TR 전송 순간에 부착한다. provider echo·예외·로그에서는 모두 제거한다 |
...
| 공급망 | GitHub Action은 full commit SHA, Gradle distribution은 SHA-256, Compose image는 manifest digest로 고정한다. resolved runtime/test/build dependency는 보안 버전 회귀 task와 SCA로 검증한다 |
| gRPC | 인증 RPC가 생기기 전 Python health server는 loopback plaintext만 허용하고 reflection은 명시 opt-in한다 |
...
| 로그 | 계좌번호, 토큰, app key는 로깅 필터에서 마스킹 |
...
| 백테스트 | 수익률/리스크 증명 |
| KIS Mock | 실제 동작 시연 |
| INTERNAL_PAPER fallback | KIS Mock 장애나 API 제한 시 시연 지속성 확보 |
...
| 3 | 데이터 수집 sample: KIS 가격, OpenDART 공시/재무, Naver/GDELT 뉴스 메타 |
...
| 데이터 원천 | KIS/OpenDART/ECOS/Naver/GDELT 필수, FRED는 후순위 보조 |
...
| 해외/뉴스 데이터 | GDELT 필수, FRED 후순위 보조 |
| KIS 공식 근거 | 한국투자증권 모의투자 안내, 한국투자증권 수수료 안내, KIS OpenAPI XLSX 20260707, `open-trading-api`, `kis-ai-extensions`, KIS MCP 문서 |
