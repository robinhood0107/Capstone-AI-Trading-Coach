...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·실패 KRX1~4 분리·KRX5 live 성공 대기(NOW 2개 endpoint만) |
...
| transport timeout/진단 | connect `2초`, read `8초`, write `2초`, pool `1초`를 각 I/O hard 상한으로, 두 NOW endpoint 전체는 `20초` logical budget으로 둔다. blocking DNS까지 강제 중단하는 wall-clock watchdog으로 주장하지 않으며 자동 retry는 계속 `0`이다. transport 예외는 문자열·request·cause를 폐기하고 `connect_timeout`, `read_timeout`, `write_timeout`, `pool_timeout`, `connect_unavailable`, `read_unavailable`, `write_unavailable`, `protocol_unavailable`, `transport_unavailable` 중 하나만 CLI에 노출함 |
...
offline fixture/mock 기반 collector 검증 뒤 KRX1~KRX4 live packet은 모두 실패 evidence로
...
operator evidence mode `0600`이다. 네 packet은 만료·소비되어 재사용하지 않고 성공 evidence와
합산하지 않는다. 현재는 31개 서비스 entitlement를 확인했지만 runtime은 NOW 두 개만 유지하며,
transport 유형별 안전 진단과 read `8초`/logical budget `20초`를 offline 회귀로 보강했다. 새
HEAD·거래일·순서·timeout profile·Redis 기준값·TTL에 결속한 KRX5 packet 승인 뒤 성공 smoke를 1회 수행해야
한다. 주기 scheduler는 아직 추가하지 않았다.
