...
`HASH-CANONICALIZATION-S22-V1`은 whitespace 없는 UTF-8 JSON, object key 사전순, stable array
sort, plain decimal, negative zero 정규화와 trailing-zero 제거를 사용한다.
`semanticInputHash`는 snapshot schema/actor/evaluation 시각, pin한 Principle ID/version/mode/
rules hash, catalog/readiness, full order intent, portfolio source/revision/owner scope/position
count, 모든 MetricKey typed state/value/unit/declared scale와 `observedAt`/`freshUntil`/source
