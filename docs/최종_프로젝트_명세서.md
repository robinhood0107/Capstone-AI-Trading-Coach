...
> 구현 상태(2026-07-15): S1.3 lower-only batch/retry, strict CLI, JSON Schema와 A3 전
> `RECOVERY0` offline 회귀 검증을 완료했다. 기존 online Approval A1과 A2는 둘 다
> 실패 evidence다. A1은 Redis reservation `+2`와 legacy counter `2`만 확정하며 exact
> physical send는 사후 확정할 수 없다. A2는 physical handoff `2`·Redis `+2` 후
> `722Y001 StatisticItemList` 응답 처리에서 실패했고 `731Y001`은 실행하지 않았다.
> `RECOVERY0` 자체의 provider 호출은 `0`회이며 다음 online gate는 packet에 묶인 신규
> Approval A3다.
...
| 수집 경계 | `StatisticSearch/{lang}/{format}/{start}/{end}/{statCode}/{cycle}/{fromDate}/{toDate}/{itemCode1}/`만 runtime polling하며 마지막 `/`를 포함한다. `ITEM_CODE2~4`, query string, 빈 placeholder는 만들지 않는다. 후보 series는 4-call TableList+ItemList preflight와 별도 activation 전까지 network 이전 차단 | News endpoint만 호출. 제목·설명·두 URL·발행시각 metadata만 허용하고 기사 URL의 DNS/GET/HEAD/redirect follow는 0건 |
...
| lifecycle | registry mismatch 시 계속 disabled | operator profile만 사용한다. 이번 S1.3 legacy 1-query smoke는 구현 검증용이고, API Hub는 disabled-ready 상태에서 2026 Q3에 NCP 계정·Application·API key ID/key와 secret entry를 준비한 뒤 2026 Q4에 pinned fixture parity와 별도 승인된 최소 1-query lifecycle 검증을 다시 수행한다. 목표 cutover `2027-03-31`, legacy rollback 제거 `2027-05-31`, hard stop `2027-06-30T00:00:00+09:00` |
...
evidence SHA는 `8b7bb4a9492d14e79234db27e86a22725f74c8415ae27347fe8c344d2d19fe27`이며
둘 다 실패 이력으로 보존한다. 다음 실행은 A3이고, 이후 A가 다시 실패하면 같은
packet으로 재실행하지 않고 A4 이상의 새 승인을 받는다. 성공 채택 집합은 A3에서
시작해 실제로 성공한 A 하나와 실제로 성공한 원자적 B 하나만으로 구성하며 A1, A2,
실패한 B를 합산하지 않는다.

별도 registry activation과 재검증, metadata 의미 승인, KRX universe audit, Naver 내부 사용·
최대 30일 보존 승인 뒤 다시 packet-bound Approval B를 받는다. B는 실행일 KST `D`의
...
이 A3 전 `RECOVERY0`에서는 public REST/gRPC, DB/Flyway, JSON Schema, dependency/lock,
Return Engine·Experience Dashboard workspace를 변경하지 않는다.
