...
> 구현 상태(2026-07-14): 아래 S1.3 lower-only batch/retry, strict CLI와 JSON Schema의 코드·offline
> 회귀 검증은 완료했다. online 검증은 별도 승인 전까지 미실행이며 실제 ECOS/Naver provider 호출은
> 0회다. Redis runtime gate는 loopback bind, 무인증 `NOAUTH`, 인증 `PONG`, AOF `yes`, 256 MiB,
> `noeviction`과 기존 named volume을 이번 실행에서 통과했다.
...
| 수집 경계 | `StatisticSearch`만 runtime polling. 후보 series는 4-call TableList+ItemList preflight와 별도 activation 전까지 network 이전 차단 | News endpoint만 호출. 제목·설명·두 URL·발행시각 metadata만 허용하고 기사 URL의 DNS/GET/HEAD/redirect follow는 0건 |
...
container-only recreate를 별도 승인받는다. 그다음 별도 승인으로 ECOS TableList·ItemList preflight를
series 2개에 대해 retry 0으로 정확히 4회 수행하고, 일치 결과를 별도 activation에 반영한 뒤 다시
승인받아 ECOS data 2회와 legacy Naver 1-query 1회를 retry 0·`--require-complete`로 실행한다.
따라서 전체 physical attempt는 정확히 `4 + 3 = 7`이다. quota count/attempt 수 불일치, registry
mismatch, Redis 실패, 429·`ERROR-602`, 최소 observation/item 미달, unsafe field 또는
schema/hash/mode 검증 실패가 발생하면 즉시 중단하며 새 승인 없이 재호출하지 않는다. 잘못된 key나
timeout 같은 negative case를 만들기 위한 live 호출은 금지하고 fixture/mock transport로만 검증한다.
...
no-replace로 게시한다. provider raw body/header/message, request URL, credential·hash, 기사 본문과
로컬 절대경로는 저장하지 않는다. 삭제 owner는
