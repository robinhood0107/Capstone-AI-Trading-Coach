...
이 amendment는 persistence/wire/CI 경계를 확정하지만 runtime Principle controller와 RiskEngine
평가 의미를 먼저 구현하지 않는다. 특히 `STRICT` 저장은 아직 evaluator 동작을 바꾸는 구현 완료를
뜻하지 않는다. 실제 endpoint는 amendment 병합 뒤 별도 최신-main implementation PR에서 catalog를
소비해 추가한다.
