...
맞아야 provider handoff가 가능하며 첫 실패 뒤 남은 호출은 0이다.
...
8. 실패 evidence와 성공 acceptance set을 합치지 않고 사용한 packet을 재사용하지 않는다. direct `curl`, 브라우저 sample, 임시 credential script로 fixed-origin transport·quota·승인 gate를 우회하지 않는다.
