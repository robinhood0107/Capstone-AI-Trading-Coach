...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·실패 KRX1~5/KRX8 분리·KRX6/7/9 만료 미실행·단일 endpoint probe/timeout 보강 완료·KRX10 단계형 live 성공 대기(NOW 2개 endpoint만) |
...
| 실행 시각 | 정상 운영은 완료 거래일 `D`의 다음 날 `08:10 KST` 이후 full refresh CLI/batch를 1회 호출한다. KRX10 recovery만 단일 승인 아래 KOSPI probe→KOSDAQ probe→full refresh 3개 명령을 순차 실행하며 이 PR은 scheduler를 추가하지 않음 |
...
합산하지 않는다. 다음 KRX10은 final HEAD·거래일·세 명령 순서·probe `2/120/2/1초+130초`·
full refresh `2/120/2/1초+260초`·발급 직전 Redis rolling-24h 기준값·TTL `60분`에 결속한다.
프로세스별 cap은 `1/1/2`이고 합계 `4`는 approval packet과 executor stop rule이 강제하며
애플리케이션이나 Redis가 approval별 합계를 직접 강제한다고 주장하지 않는다. 어느 단계든 실패하면
남은 명령·추가 호출·자동 재시도는 `0`이다. 주기 scheduler는 아직 추가하지 않았다.
