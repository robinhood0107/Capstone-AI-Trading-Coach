...
packet은 `orderDivision`과 선택적 `exchangeDivision`도 결속한다. `exchangeDivision=NXT`는
지정가·보통(`orderDivision=00`) 1주 probe에만 허용하며, 생략하면 `KRX`가 기본이다. 같은 값은
주문 submit, encrypted cancel reference, 전량취소, 최근 체결조회 source-shape probe에 적용된다.
