...
> 회귀 검증을 완료했다. online Approval A1·A2·A3는 모두 실패 evidence로 분리한다.
> A3는 physical handoff `2`·Redis `+2` 후 `722Y001 StatisticItemList`의 동일
> `ITEM_CODE` 후보 `4`건(`candidate_duplicate`)에서 안전하게 중단했고 `731Y001`은
> 실행하지 않았다. 공식 metadata 차원에 맞춰 ItemList 후보를 `STAT_CODE`·
> `GRP_CODE=Group1`·`ITEM_CODE`·요청 `CYCLE` 완전 일치로 한정하는 회귀 테스트와
> parser 수정을 추가했으며, 이 복구 작업의 provider 호출은 `0`회다. 다음 online
> 실행은 전체 offline gate를 통과한 새 HEAD에 묶인 신규 Approval A4만 허용한다.
...
같은 packet을 재사용하지 않는다. 다음 실행은 새 HEAD·명령·series·TTL에 묶인 A4다.
성공 채택 집합은 실제로 성공한 A 하나와 실제로 성공한 원자적 B 하나만으로 구성하며
A1·A2·A3와 실패한 B를 합산하지 않는다.
...
A3 후 A4 복구 변경에서는 public REST/gRPC, DB/Flyway, JSON Schema, dependency/lock,
Return Engine·Experience Dashboard workspace를 변경하지 않는다. A4는 회귀·전체 gate와
독립 리뷰를 통과한 새 packet을 사용자가 정확히 승인하기 전에는 실행하지 않는다.
