...
| KRX OPEN API | S1.3K 국내주식 universe 생성용 일별 시가총액·거래대금 | 별도 PR offline 구현 완료·live 승인 대기(NOW 2개 endpoint만) |
...
### 11.1.4 S1.3K KRX OPEN API universe 자동화 (별도 PR offline 구현·검증 완료, live 미실행)
...
브랜치에서 구현했고, KRX 인증키·NOW 두 서비스 활용 승인·신규 KRX online 승인 packet이
확인되기 전까지 live 호출은 실행하지 않는다. 공식 근거는 [KRX OPEN API 서비스 목록](https://openapi.krx.co.kr/contents/OPP/INFO/service/OPPINFO004.cmd),
...
- `NEXT`: 현재 상세명세의 universe·KOSPI 상대강도·금 ETF/ETN 위험 사용처가 있지만,
  S1.3K에서는 disabled catalog로만 두고 별도 테스트·서비스 승인 후 연다.
...
수량은 `NOW 2 + NEXT 7 + LATER 13 + EXCLUDE 9 = 31`로 고정한다. NOW 이외 API ID는 개발명세서를
미리 분석할 수는 있지만 서비스 활용 신청, credential 주입, endpoint 호출, persisted artifact
생성은 각 상태의 별도 세션이 승인되기 전까지 금지한다.

#### NOW 수집·스케줄·fallback 계약

| 항목 | 계약 |
|---|---|
| 입력 | 공식 제공 시작일 `2010-01-04` 이상 완료 거래일 `D`의 exact `basDd=YYYYMMDD`. 명시한 `--as-of`는 XKRX 거래일·안전 최신일 이하여야 하고, 생략하면 로컬 XKRX calendar와 `08:10 KST` 안전 경계로 최신 가용일을 정함. 범위 검사는 calendar 호출보다 먼저 수행하고 calendar 계산 실패는 client 0건의 안정 오류로 종료함. 경계 전에는 주말·휴일에도 최근 session을 한 단계 더 제외하며 provider 실패를 이전 날짜 재호출로 바꾸지 않음 |
...
| 저장 | provider raw body/header/request URL은 저장하지 않음. ignored Decision Platform root의 기존 `UniverseManifest` v1과 Markdown report에 고정 source label, 기준일, 검증된 전체 canonical row SHA-256, 선정 30개를 기록함. provider 종목명의 Markdown 구두점은 표·링크·이미지 문법으로 해석되지 않게 escape함. client cleanup 성공 뒤 report→manifest 순서로 게시하고, CLI는 로컬 경로 없이 안정 code와 physical attempt 수만 출력함. 별도 provenance 파일은 만들지 않음 |
...
현재 상태는 offline fixture/mock 기반 collector 검증까지다. KRX DNS·provider·credential을 쓰는
live smoke와 주기 scheduler는 실행·추가하지 않았으며, 이를 구현 완료 근거와 섞지 않는다.
