...
| S1.2~S1.4 analysis data | S1.2 OpenDART 공시, S1.3 ECOS 거시/Naver 뉴스 메타, S1.4 수익률·리스크 지표 계산 모듈 | KIS 외 데이터와 순수 계산 위주로 순차 구현. KIS 순위/업종/지수 시세는 필요 시 후속 planning으로만 검토 |
...
KIS XLSX 20260707 기준으로 문서화할 주요 API는 다음과 같다. S1.1 구현 대상은 인증과 시장데이터 조회에 한정하고, 주문·잔고·주문가능 계열은 S3 Brokerage까지 구현하지 않는다.
