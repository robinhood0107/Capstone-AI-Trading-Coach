...
mapfile -d '' shell_files < <(git ls-files -z -- '*.sh' ':(exclude)private-reference/**')
((${#shell_files[@]} > 0))
...
mapfile -d '' dockerfiles < <(git ls-files -z -- '*Dockerfile*' ':(exclude)private-reference/**')
((${#dockerfiles[@]} > 0))
hadolint "${dockerfiles[@]}"

mapfile -d '' yaml_files < <(git ls-files -z -- '*.yml' '*.yaml' ':(exclude)private-reference/**')
((${#yaml_files[@]} > 0))
uv --project "$python_project" run yamllint -c .yamllint.yml "${yaml_files[@]}"

mapfile -d '' toml_files < <(git ls-files -z -- '*.toml' ':(exclude)private-reference/**')
((${#toml_files[@]} > 0))
