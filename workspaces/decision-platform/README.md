...
1. `cp ../../.env.example ../../.env` 후 PostgreSQL/collector/source-writer/Redis password, role별 offline DSN, JWT issuer/audience, 목적별 JWT/login/credential HMAC key, single-quoted attested demo credential bundle과 필요한 provider secret을 채운다. plaintext demo password는 `.env`에 저장하지 않는다.
...
event/source-ref 각 100개 상한을 사용한다. 초과는 truncate된 성공이 아니라 technical failure다.
...
사용하지 않는다. provider credential은 필요하지 않고 provider/live/account/order/broker call은
모두 0이다. Spring/Python process에는 루트 `.env`의 필요한 값을 IDE나 local secret manager의
env-file 기능으로 주입하되 파일 내용을 shell command로 출력하거나 추적 파일에 복사하지 않는다.
