...
def main(argv: list[str] | None = None) -> int:
    ...
    report_path = (
        ...
        if args.report_path
        else settings.data_dir / "reports" / "kis_s1_1_report.md"
    )
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--symbols", help="Comma or space separated stock codes. Defaults to KOSPI large-cap seed."
    )
    parser.add_argument(
        "--symbols-file", help="Text/CSV file. First column is treated as stock code."
    )
    parser.add_argument(
        "--universe-manifest", help="Universe manifest JSON generated by kis-universe-refresh."
    )
...
def _resolve_symbols(args: argparse.Namespace, data_dir: Path) -> _SymbolResolution:
    ...
    manifest_path = (
        ...
        else data_dir / "universe_manifest.json"
    )
