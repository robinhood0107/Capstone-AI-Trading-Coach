...
_STAGE_FUNCTION = "public.stage_rag_v2_immutable_public_bge_document(jsonb)"
_EVALUATE_FUNCTION = "public.evaluate_rag_v2_immutable_public_bge_component(text,jsonb)"
...
PublicBgeRecord: TypeAlias = tuple[RagV2BgeMaterializedPublicDocument, PublicBgeSourceMetadata]
PublicBgeComponentScope: TypeAlias = Literal["EXACT30", "OA112"]


class PublicBgeStagingRepositoryError(ValueError):
    """public BGE staging/evaluation의 capability 또는 immutable receipt 경계 실패다."""


@dataclass(frozen=True, slots=True)
class RagV2PublicBgeStagingReceipt:
    """public source 한 건의 content-free resumable staging receipt다."""

    component_generation_id: str
    materialization_run_id: str
    component_scope: PublicBgeComponentScope
...
@dataclass(frozen=True, slots=True)
class PublicBgeEvaluationEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise PublicBgeStagingRepositoryError("PUBLIC_BGE_EVALUATION_ARGUMENT")
...
@dataclass(frozen=True, slots=True)
class RagV2PublicBgeEvaluationReceipt:
...
class PsycopgRagV2PublicBgeStagingRepository:
    """`decision_rag_writer` DSN로 public BGE writer capability만 호출한다.
    ...
    def __init__(self, *, database_dsn: str) -> None:
        if not database_dsn or len(database_dsn) > 4_096:
            raise PublicBgeStagingRepositoryError("PUBLIC_BGE_STAGE_DATABASE_DSN")
        self._database_dsn = database_dsn

    def stage(
        self,
        *,
        record: PublicBgeRecord,
        ...
        payload = build_public_bge_staging_payload(record, context=context)
        try:
            with psycopg.connect(
                ...
                with connection.transaction():
                    ...
                    row = connection.execute(
                        ...
                        FROM public.stage_rag_v2_immutable_public_bge_document(%s::jsonb)
                        """,
                        (Jsonb(payload),),
                    ).fetchone()
            ...
            raise PublicBgeStagingRepositoryError("PUBLIC_BGE_STAGE_REJECTED") from None
        return _staging_receipt(row, context=context)

    def stage_component(
        self,
        *,
        records: Sequence[PublicBgeRecord],
        context: RagV2PublicBgeComponentContext,
    ) -> tuple[RagV2PublicBgeStagingReceipt, ...]:
        ...
        receipts: list[RagV2PublicBgeStagingReceipt] = []
        try:
            with psycopg.connect(
                self._database_dsn,
                autocommit=True,
    ...
    def evaluate(
        ...
        *,
    ...
    ) -> RagV2PublicBgeEvaluationReceipt:
        """complete local BGE component의 aggregate evaluation evidence만 immutable로 기록한다.
        ...
        try:
            ...
            raise PublicBgeStagingRepositoryError("PUBLIC_BGE_EVALUATION_REJECTED") from None
...
def _validate_context(context: RagV2PublicBgeComponentContext) -> None:
    ...
    if (
        ...
        raise PublicBgeStagingRepositoryError("PUBLIC_BGE_STAGE_COMPONENT_CONTEXT")
...
def _evaluation_receipt(
    ...
    *,
    ...
    if (
        ...
        raise PublicBgeStagingRepositoryError("PUBLIC_BGE_EVALUATION_RECEIPT")
    return RagV2PublicBgeEvaluationReceipt(
        component_generation_id=row[0],
        component_scope=context.component_scope,
        embedding_profile_id="bge_m3_local_1024_v1",
        state="EVALUATED",
    ...
    for table in _WRITER_FORBIDDEN_TABLES:
        for privilege in ("SELECT", "INSERT", "UPDATE", "DELETE", "TRUNCATE", "REFERENCES", "TRIGGER"):
            ...
            if row is not None and row[0] is True:
                raise PublicBgeStagingRepositoryError("PUBLIC_BGE_STAGE_WRITER_PRIVILEGE")
