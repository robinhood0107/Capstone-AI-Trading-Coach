...
낮은 55K request cap과 32K contextual segment cap을 함께 적용한다. 55K cap은 이미
`UNKNOWN_BILLING`으로 종료된 110K·60K plan과 identity를 분리해 append-only evidence를 보존하면서
forward recovery를 허용한다. Provider가 추가하는 document prompt의 usage는 별도 120K packet cap으로 닫는다.
"""
...
# 소비된 60K plan을 재사용하지 않고 response/request 크기를 함께 줄이는 forward-only cap이다.
_TOKEN_CAP: Final = 55_000
...
def build_public_voyage_batch_plan(
    *,
    ...
    """public base를 55K 이하의 deterministic ordered first-fit batch set으로 만든다.
