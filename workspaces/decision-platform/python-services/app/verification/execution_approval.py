...
class P1ExecutionApproval:
    ...
    def validate(self, *, now: datetime | None = None) -> None:
        ...
        if (not self.exact_operations or len(set(self.exact_operations)) != len(self.exact_operations)
        ...
        digests = (self.payload_sha256, self.repository_digest, self.evidence_digest,
                   self.owner_scope_digest, self.account_scope_digest,
        ...
        if (type(self.physical_call_cap) is not int or not 1 <= self.physical_call_cap <= 112
...
def author_execution_approval(*, approval_id: str, issuer_key_id: str,
        private_key_path: Path, provider_family: str,
        exact_operations: Sequence[str], payload_sha256: str,
        repository_digest: str, evidence_digest: str,
        owner_scope_digest: str, account_scope_digest: str,
        credential_scope_digest: str, physical_call_cap: int,
        cost_cap_microusd: int, now: datetime) -> P1ExecutionApproval:
    ...
    unsigned = P1ExecutionApproval(
        approval_id, issuer_key_id, secrets.token_hex(16), provider_family,
        tuple(exact_operations), payload_sha256, repository_digest,
        evidence_digest, owner_scope_digest, account_scope_digest,
        credential_scope_digest, physical_call_cap, cost_cap_microusd,
        now.astimezone(UTC) + timedelta(minutes=5), "A" * 86,
    )
    unsigned.validate(now=now)
    signature = _b64url(_load_private_key(private_key_path).sign(
        canonical_json_bytes(unsigned.signing_body())))
    return replace(unsigned, signature=signature)


def load_and_verify_execution_approval(approval_path: Path, *,
        provider_family: str, exact_operations: Sequence[str],
        payload_sha256: str, repository_digest: str, evidence_digest: str,
        physical_call_cap: int, cost_cap_microusd: int, now: datetime,
        owner_scope_digest: str = ZERO_SCOPE_SHA256,
        account_scope_digest: str = ZERO_SCOPE_SHA256,
        credential_scope_digest: str = ZERO_SCOPE_SHA256) -> P1ExecutionApproval:
    ...
    observed = (approval.provider_family, approval.exact_operations,
        approval.payload_sha256, approval.repository_digest,
        approval.evidence_digest, approval.owner_scope_digest,
        approval.account_scope_digest, approval.credential_scope_digest,
        approval.physical_call_cap, approval.cost_cap_microusd)
    expected = (provider_family, tuple(exact_operations), payload_sha256,
        repository_digest, evidence_digest, owner_scope_digest,
        account_scope_digest, credential_scope_digest, physical_call_cap,
        cost_cap_microusd)
    ...
    try:
        _load_public_key(public_bytes).verify(_decode(approval.signature),
            canonical_json_bytes(approval.signing_body()))
...
def execution_approval_from_dict(value: Mapping[str, object]) -> P1ExecutionApproval:
    expected = {"accountScopeDigest", "approvalId", "contractId", "costCapMicrousd",
        "credentialScopeDigest", "evidenceDigest", "exactOperations", "expiresAt",
        "issuerKeyId", "nonce", "ownerScopeDigest", "payloadSha256",
        "physicalCallCap", "providerFamily", "repositoryDigest", "signature"}
    operations = value.get("exactOperations")
    strings = expected - {"costCapMicrousd", "exactOperations", "physicalCallCap"}
    if (set(value) != expected or value.get("contractId") != _CONTRACT_ID
    ...
    try:
        packet = P1ExecutionApproval(
            cast(str, value["approvalId"]), cast(str, value["issuerKeyId"]),
            cast(str, value["nonce"]), cast(str, value["providerFamily"]),
            tuple(cast(list[str], operations)), cast(str, value["payloadSha256"]),
            cast(str, value["repositoryDigest"]), cast(str, value["evidenceDigest"]),
            cast(str, value["ownerScopeDigest"]), cast(str, value["accountScopeDigest"]),
            cast(str, value["credentialScopeDigest"]), cast(int, value["physicalCallCap"]),
            cast(int, value["costCapMicrousd"]), _parse_instant(cast(str, value["expiresAt"])),
