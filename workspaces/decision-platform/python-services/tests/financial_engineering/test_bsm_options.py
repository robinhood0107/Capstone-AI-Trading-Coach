...
CANONICAL = dict(underlying_price=72_000.0, strike=75_000.0, tau=0.25, risk_free_rate=0.032, dividend_yield=0.01, volatility=0.28)


def _independent_price(right: str, *, s: float, k: float, tau: float, r: float, q: float, sigma: float) -> float:
...
def test_independent_scipy_oracle_q_parity_and_bounds(right: str) -> None:
    ...
    expected = _independent_price(right, s=CANONICAL["underlying_price"], k=CANONICAL["strike"], tau=CANONICAL["tau"], r=CANONICAL["risk_free_rate"], q=CANONICAL["dividend_yield"], sigma=CANONICAL["volatility"])
    lower, upper = discounted_no_arbitrage_bounds(option_right=right, **{key: value for key, value in CANONICAL.items() if key != "volatility"})
    ...
    parity = CANONICAL["underlying_price"] * math.exp(-CANONICAL["dividend_yield"] * CANONICAL["tau"]) - CANONICAL["strike"] * math.exp(-CANONICAL["risk_free_rate"] * CANONICAL["tau"])
...
def test_valid_monotonicities_without_blanket_tau_claim() -> None:
    ...
    assert black_scholes(option_right="CALL", **{**CANONICAL, "underlying_price": 73_000.0}).theoretical_price > base
    assert black_scholes(option_right="CALL", **{**CANONICAL, "strike": 76_000.0}).theoretical_price < base
    assert black_scholes(option_right="CALL", **{**CANONICAL, "volatility": 0.3}).theoretical_price > base
...
def test_all_analytic_greeks_match_central_finite_differences(right: str) -> None:
    ...
    delta = (price(underlying_price=CANONICAL["underlying_price"] + h_s) - price(underlying_price=CANONICAL["underlying_price"] - h_s)) / (2 * h_s)
    gamma = (price(underlying_price=CANONICAL["underlying_price"] + h_s) - 2 * price() + price(underlying_price=CANONICAL["underlying_price"] - h_s)) / h_s**2
    vega = (price(volatility=CANONICAL["volatility"] + h_v) - price(volatility=CANONICAL["volatility"] - h_v)) / (2 * h_v)
    rho = (price(risk_free_rate=CANONICAL["risk_free_rate"] + h_r) - price(risk_free_rate=CANONICAL["risk_free_rate"] - h_r)) / (2 * h_r)
...
def test_iv_round_trip_endpoint_bounds_bracket_and_iteration_failure() -> None:
    ...
    result = implied_volatility(option_right="CALL", market_price=market, **{key: value for key, value in CANONICAL.items() if key != "volatility"})
    assert result.implied_volatility == pytest.approx(0.28, abs=1e-8)
    assert result.warning == "CALIBRATION_IDENTITY"
    assert implied_volatility(option_right="PUT", market_price=black_scholes(option_right="PUT", **{**CANONICAL, "volatility": LOWER_VOLATILITY}).theoretical_price, **{key: value for key, value in CANONICAL.items() if key != "volatility"}).implied_volatility == LOWER_VOLATILITY
    assert implied_volatility(option_right="CALL", market_price=black_scholes(option_right="CALL", **{**CANONICAL, "volatility": UPPER_VOLATILITY}).theoretical_price, **{key: value for key, value in CANONICAL.items() if key != "volatility"}).implied_volatility == UPPER_VOLATILITY
    lower, upper = discounted_no_arbitrage_bounds(option_right="CALL", **{key: value for key, value in CANONICAL.items() if key != "volatility"})
    with pytest.raises(ValueError, match="IV_NOT_BRACKETED"):
        implied_volatility(option_right="CALL", market_price=upper + 1, **{key: value for key, value in CANONICAL.items() if key != "volatility"})
    with pytest.raises(ValueError, match="IV_NOT_CONVERGED"):
        implied_volatility(option_right="CALL", market_price=market, max_iterations=1, tolerance=1e-12, **{key: value for key, value in CANONICAL.items() if key != "volatility"})
    assert lower >= 0


def test_secondary_2315_42_fixture_has_correct_iv() -> None:
    result = implied_volatility(option_right="CALL", market_price=2315.42, tolerance=1e-10, **{key: value for key, value in CANONICAL.items() if key != "volatility"})
    assert result.implied_volatility == pytest.approx(0.237005877501, abs=1e-10)


@pytest.mark.parametrize("field,value", [("tau", 0.0), ("volatility", 0.0), ("underlying_price", np.nan), ("strike", np.inf)])
...
def test_near_zero_off_kink_converges_without_atm_singularity_convention() -> None:
    call = black_scholes(option_right="CALL", underlying_price=120, strike=100, tau=1e-8, risk_free_rate=0.01, dividend_yield=0, volatility=1e-4)
    assert call.theoretical_price == pytest.approx(20.0, rel=1e-7)
    with pytest.raises(ValueError, match="bsm_domain_invalid"):
        option_greeks(option_right="CALL", underlying_price=100, strike=100, tau=0, risk_free_rate=0, dividend_yield=0, volatility=0.2)
