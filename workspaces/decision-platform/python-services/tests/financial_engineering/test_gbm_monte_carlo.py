...
def test_seed_and_max_draw_prefix_are_exactly_reproducible() -> None:
    kwargs = dict(s0=100.0, mu_sde=0.04, sigma=0.2, horizon=1.0, dt=1 / 252, seed=19)
    maximum = generate_terminal_prices(**kwargs, n_paths=10_000)
    assert generate_terminal_prices(**kwargs, n_paths=1_000) == pytest.approx(maximum[:1_000], abs=0)
    assert generate_terminal_prices(**kwargs, n_paths=5_000) == pytest.approx(maximum[:5_000], abs=0)
...
def test_terminal_mean_and_loss_probability_match_lognormal_oracle() -> None:
    ...
    terminal = generate_terminal_prices(s0=s0, mu_sde=mu, sigma=sigma, horizon=horizon, dt=1 / 252, n_paths=10_000, seed=20260821)
    expected_mean = s0 * math.exp(mu * horizon)
    expected_loss_probability = float(ndtr((math.log(s0 / s0) - (mu - sigma * sigma / 2) * horizon) / (sigma * math.sqrt(horizon))))
...
def test_twenty_batch_se_ci_and_warn_gates_are_reported_without_auto_growth() -> None:
    report = run_gbm_monte_carlo(s0=100.0, mu_sde=0.0, sigma=0.7, horizon=1.0, dt=1 / 252, n_paths=1_000, seed=7)
    final = report.prefix_metrics[-1]
    assert final.path_count == 1_000
    assert final.var_loss95_return.interval95.lower <= final.var_loss95_return.value <= final.var_loss95_return.interval95.upper
...
def test_stochastic_and_deterministic_result_types_cannot_be_confused() -> None:
    ...
    stress = deterministic_stress(jump_gap=-0.1, fat_tail_proxy=-0.2, volatility_cluster_burst=-0.08, leverage_margin_shortfall=0.03, liquidity_spread_impact_crowding=-0.04)
    ...
    with pytest.raises(ValueError, match="crisis_correlation_requires_portfolio"):
        deterministic_stress(jump_gap=0, fat_tail_proxy=0, volatility_cluster_burst=0, leverage_margin_shortfall=0, liquidity_spread_impact_crowding=0, crisis_correlation=0.8)
...
def test_domain_finite_and_resource_caps(kwargs: dict[str, object], code: str) -> None:
    inputs: dict[str, object] = {"s0": 100.0, "mu_sde": 0.04, "sigma": 0.2, "horizon": 1.0, "dt": 1 / 252, "n_paths": 1_000, "seed": 1}
...
def test_10k_hard_cap_runtime_and_memory_are_bounded() -> None:
    ...
    report = run_gbm_monte_carlo(s0=100.0, mu_sde=0.04, sigma=0.2, horizon=1.0, dt=1 / 252, n_paths=10_000, seed=3)
