...
def _brute_force_posterior(observations: np.ndarray, start: np.ndarray, transition: np.ndarray, means: np.ndarray, covars: np.ndarray) -> np.ndarray:
    ...
    for end in range(1, len(observations) + 1):
        ...
        for states in itertools.product(range(2), repeat=end):
            ...
            for index, state in enumerate(states):
                ...
                emission = math.exp(-0.5 * float(np.sum(diff * diff / covars[state]))) / math.sqrt(float(np.prod(2 * math.pi * covars[state])))
...
def test_manual_forward_matches_brute_force_tiny_oracle() -> None:
    ...
    actual, _ = _forward_posteriors(observations, start_probability=start, transition_probability=transition, means=means, covariances=covars)
    assert actual == pytest.approx(_brute_force_posterior(observations, start, transition, means, covars), abs=1e-12)
...
class _FakeModel:
    def __init__(self, seed: int, *, fail: bool = False, likelihood: float | None = None) -> None:
        self.seed = seed
        self.fail = fail
        self.monitor_ = _Monitor(not fail, ((likelihood if likelihood is not None else float(seed)),))
...
def _balanced_closes(rows: int = 600) -> np.ndarray:
    ...
    returns = np.concatenate([rng.normal(-0.002, 0.008, rows // 2), rng.normal(0.003, 0.025, rows // 2)])
...
def test_manual_multistart_selects_likelihood_then_smaller_seed() -> None:
    ...
    result = fit_hmm_regime(_balanced_closes(), train_rows=500, model_factory=lambda seed: _FakeModel(seed, likelihood=likelihoods[seed]))
    assert result.availability == "AVAILABLE"
    assert result.artifact is not None
    assert result.artifact.selected_seed == 29
    assert result.artifact.label_by_internal_state == ("RISK_ON", "RISK_OFF")
    assert all(math.isfinite(item.normalized_entropy) and 0 <= item.normalized_entropy <= 1 for item in result.observations)
...
def test_every_failed_candidate_abstains_without_artifact() -> None:
    result = fit_hmm_regime(_balanced_closes(), train_rows=500, model_factory=lambda seed: _FakeModel(seed, fail=True))
...
@pytest.mark.parametrize(
    ("mutation", "expected"),
    [
        (lambda model: setattr(model, "_covars_", np.full((2, 2), MIN_COVAR / 2)), "covariance_below_minimum"),
        (lambda model: setattr(model, "transmat_", np.array([[0.9, 0.2], [0.1, 0.9]])), "transition_invalid"),
        (lambda model: setattr(model, "startprob_", np.array([float("nan"), 1.0])), "start_probability_invalid"),
    ],
)
...
def test_real_fit_is_bounded_and_artifact_contains_no_serialized_object() -> None:
    ...
    if result.artifact:
        ...
        assert "pickle" not in serialized and "joblib" not in serialized and "cloudpickle" not in serialized
