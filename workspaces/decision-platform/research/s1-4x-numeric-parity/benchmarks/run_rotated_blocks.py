#!/usr/bin/env python3
"""S1.4X의 3회전 87개 native family block을 무덮어쓰기 방식으로 실행한다."""
...
def _strict_json_load_bytes(payload: bytes) -> Any:
    ...
    try:
        ...
        raise ContractError("INVALID_COMMAND_MANIFEST_JSON") from exc
...
def _verify_host_validity_report(
    ...
    *,
    ...
    if path.is_symlink() or not path.is_file():
        raise ContractError("HOST_VALIDITY_ARTIFACT_MISSING")
    first_sha256 = sha256_file(path)
    report = strict_json_load(path)
    if sha256_file(path) != first_sha256:
        raise ContractError("HOST_VALIDITY_ARTIFACT_CHANGED_DURING_READ")
    if (
...
def _terminate_process_group(process: subprocess.Popen[bytes]) -> None:
    ...
    leader_reaped = _wait_for_leader(process, timeout_seconds=5)
    group_exited = _wait_for_process_group_exit(process.pid, 5)
