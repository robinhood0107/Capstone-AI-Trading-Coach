...
"$S1_4X/integration/tools/launch-detached-full-run.sh" \
  ...
  --scala-qualification-source \
    "$HOME/.cache/s1-4x/workflow-runs/native-oci-01bfbaa-final1/scala" \
  --haskell-static-source \
    "$HOME/.cache/s1-4x/workflow-runs/native-oci-01bfbaa-final1/haskell" \
  --haskell-profile-source \
    "$HOME/.cache/s1-4x/workflow-runs/haskell-p-a30bbca-final1"
```

이 sealed continuation에만 `S1_4X_IGNORE_AMBIENT_HOST_ACTIVITY=1`이 supervisor 내부에서
설정된다. 실행 컨테이너 수와 외부 workflow CPU 기준은 observe-only가 되고, Docker API,
