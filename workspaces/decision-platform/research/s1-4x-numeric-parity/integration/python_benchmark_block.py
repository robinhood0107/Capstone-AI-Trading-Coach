...
def _consume(value: object) -> float:
    ...
    if hasattr(value, "block_until_ready"):
        value = value.block_until_ready()  # type: ignore[union-attr]
...
def _numpy_research_operation(
    ...
    if function_id == "probabilistic_sharpe_ratio":
        ...
        def probabilistic_batch() -> float:
            return sum(
                numpy_reference.probabilistic_sharpe_ratio(float(value), **arguments)
...
def _jax_research_operation(
    ...
    *,
    ...
    from s1_4r_risk_research import _jax_kernels  # type: ignore[import-not-found]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        # Import, decode, device transfer/JIT compile, validation과 첫 강제 평가는 timing 밖이다.
        ...
        mark_measurement_entered(arguments.qualification.resolve(strict=True))
        started = _utc_now()
        native_cases = []
        measured_cases = []
