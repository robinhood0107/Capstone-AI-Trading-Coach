...
/** 20개 numeric API가 공유하는 순차 Float64 primitive다. 순서 변경·FMA·병렬 reduction 없이 frozen Python/JAX 기준의
  * branch와 안정 오류 계약을 보존한다.
  */
object NumericPrimitives:
  ...
  private def horner(argument: Double, coefficients: Vector[Double]): Double =
    coefficients.foldLeft(0.0)((accumulator, coefficient) => accumulator * argument + coefficient)
    ...
    if math.abs(q) <= 0.425 then
      ...
      val numerator =
        horner(
          r,
          Vector(
            2.5090809287301227e3, 3.3430575583588128e4, 6.72657709270087e4, 4.592195393154987e4,
            1.373169376550946e4, 1.9715909503065514e3, 1.3314166789178438e2, 3.3871328727963665
          )
        ) * q
      val denominator =
        horner(
          r,
          Vector(
            5.226495278852855e3, 2.872908573572194e4, 3.930789580009271e4, 2.1213794301586597e4,
            5.394196021424751e3, 6.871870074920579e2, 4.231333070160091e1, 1.0
          )
        )
        ...
        if root <= 5.0 then
          ...
          val numerator =
            horner(
              r,
              Vector(
                7.745450142783414e-4, 2.2723844989269185e-2, 2.417807251774506e-1,
                1.2704582524523684, 3.6478483247632045, 5.769497221460691, 4.630337846156545,
                1.4234371107496835
              )
            )
          val denominator =
            horner(
              r,
              Vector(
                1.0507500716444169e-9, 5.475938084995345e-4, 1.5198666563616457e-2,
                1.4810397642748007e-1, 6.897673349851e-1, 1.6763848301838038, 2.0531916266377588,
                1.0
              )
            )
          numerator / denominator
          ...
          val numerator =
            horner(
              r,
              Vector(
                2.010334399292288e-7, 2.7115555687434876e-5, 1.2426609473880784e-3,
                2.6532189526576123e-2, 2.965605718285049e-1, 1.7848265399172913, 5.463784911164114,
                6.657904643501103
              )
            )
          val denominator =
            horner(
              r,
              Vector(
                2.0442631033899397e-15, 1.4215117583164459e-7, 1.8463183175100548e-5,
                7.868691311456133e-4, 1.4875361290850615e-2, 1.369298809227358e-1,
                5.99832206555888e-1, 1.0
              )
            )
          numerator / denominator
  ...
  def likelihoodRatio(nullLog: Double, alternativeLog: Double): Either[StableError, Double] =
    if !nullLog.isFinite || !alternativeLog.isFinite then Left(StableError.ResearchResultNonFinite)
    else
