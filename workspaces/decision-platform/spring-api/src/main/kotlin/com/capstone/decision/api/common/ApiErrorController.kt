...
// 왜: 서블릿 컨테이너가 만든 /error 응답도 API 영역에서는 빈 404/500이 아니라 envelope여야 한다.
@RestController
class ApiErrorController : ErrorController {
    ...
    fun handleError(request: HttpServletRequest): ResponseEntity<ApiResponse<Nothing>> {
        ...
        // 왜: 미매핑 경로를 details에 남겨 swagger/manual smoke에서 원인을 바로 볼 수 있게 한다.
        val details =
            mapOf(
                "path" to (
    ...
    // 왜: 컨테이너 status를 공개 error.code로 재매핑해 클라이언트 분기 규칙을 유지한다.
