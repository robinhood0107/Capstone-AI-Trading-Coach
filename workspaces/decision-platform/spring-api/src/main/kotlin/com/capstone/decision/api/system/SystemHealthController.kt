...
// swagger smoke와 프론트 상태 배지가 인증된 공통 envelope를 검증할 최소 API가 필요하다.
@RestController
@RequestMapping("/api/v1/system")
class SystemHealthController {
    @GetMapping("/health")
    fun health(): SystemHealthResponse =
        // S0.3은 walking skeleton이므로 외부 서비스 연결 전 고정 UP 상태로 계약 형태만 잠근다.
...
// API 명세 2.7의 상태 응답 형태를 코드 타입으로 먼저 고정한다.
