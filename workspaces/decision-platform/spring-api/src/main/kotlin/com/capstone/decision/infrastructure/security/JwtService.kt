...
// 왜: token 발급/검증 책임을 분리해 controller와 filter가 JWT 구현 세부사항을 모르도록 한다.
@Service
class JwtService(
    private val properties: JwtProperties,
) {
    // 왜: 테스트에서 토큰 만료 계산이 한 지점으로 모이도록 Clock을 명시한다.
    ...
    fun issue(account: DemoAccount): IssuedToken {
        ...
        val token =
            // 왜: JWT에는 userId와 role만 담아 민감정보가 토큰 payload에 들어가지 않게 한다.
            Jwts
                .builder()
    ...
    fun parse(token: String): AppPrincipal {
        val claims =
            // 왜: 같은 signing key로 검증한 토큰만 SecurityContext에 올려 forged token을 차단한다.
            Jwts
                .parser()
    ...
    private fun signingKey(): SecretKey {
        // 왜: HS256은 짧은 secret을 허용하면 brute force 위험이 커서 시작 시 바로 실패시킨다.
...
// 왜: swagger smoke가 만료 시각까지 확인할 수 있도록 발급 결과를 명시적으로 묶는다.
