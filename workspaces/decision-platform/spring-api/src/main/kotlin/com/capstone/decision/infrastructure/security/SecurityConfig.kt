...
// 왜: S0.3 공통 규약에서 허용 경로, JWT 인증, CORS를 한 보안 체인으로 고정한다.
@Configuration
@EnableWebSecurity
...
class SecurityConfig {
    ...
    fun securityFilterChain(
        ...
        val jwtAuthenticationFilter =
            // 왜: Redis idempotency를 별도 filter로 늘리지 않고 JWT 인증 뒤 write gate로 연결한다.
        ...
        return http
            // 왜: Bearer API에서는 브라우저 세션/폼 인증 상태를 만들지 않는다.
            ...
            .exceptionHandling { exceptionHandling ->
                ...
                authorize
                    ...
                    .permitAll()
                authorize
                    // 왜: 로그인, 헬스체크, 문서 endpoint는 토큰 발급 전에도 접근되어야 한다.
                    .requestMatchers(
                        "/actuator/health",
                        "/swagger-ui/**",
                        "/swagger-ui.html",
    ...
    fun corsConfigurationSource(): CorsConfigurationSource {
        // 왜: dashboard 로컬 개발 origin만 열어 S0.3 smoke와 최소 보안 경계를 함께 만족한다.
        val configuration =
