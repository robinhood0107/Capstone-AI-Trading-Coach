-- 위험 판정이 ALLOW가 아닐 때 run을 정상적으로 닫을 수 없다.
-- `p1_advance_automation_checkpoint_v2`는 예약이 있는 전이에서 `p_decision_id`가 오면 언제나
-- 그 판단이 `ALLOW`이고 주문 가능하기를 요구한다. 그런데 `ORDER_SIZING`에서 예약을 만든 뒤
-- `RISK_CHECKING`에서 HOLD를 받으면 엔진은 예약을 그대로 든 채 `SKIPPED_NO_ACTION`으로 전이한다.
-- 그 전이도 같은 검사에 걸려 40001 `automation exact Decision intent mismatch`로 죽는다.
--
-- 즉 위험 판정이 매수를 거절하는 정상 경로가 기록되지 못하고 매번 예외로 끝난다. 이름과 달리
-- 실제로 어긋난 것은 intent가 아니라 outcome이라 원인도 오해하기 쉽다.
--
-- 검사의 목적은 "주문은 그 정확한 intent에 결속된 ALLOW 판단 아래에서만 나간다"이므로, 주문을
-- 여는 전이에서만 요구하도록 범위를 좁힌다. 주문이 실제로 나가는 경로의 결속은 그대로다.
-- 나머지 본문은 V99와 같다.
