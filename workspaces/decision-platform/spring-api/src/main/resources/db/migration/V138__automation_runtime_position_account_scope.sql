-- Keep runtime position snapshots within the active brokerage account.
...
BEGIN
  ...
  WHERE position.user_id=claim_row.user_id
    AND position.account_id=control_row.account_id AND position.policy_id IS NOT NULL;
...
BEGIN
  ...
  FROM public.automation_positions_effective position
  WHERE position.user_id=claim_row.user_id
    AND position.account_id=control_row.account_id AND position.max_holding_sessions IS NOT NULL
