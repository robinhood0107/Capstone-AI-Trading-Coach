-- Writers must use the same preserved policy and account scope as runtime readers.
...
BEGIN
  ...
  IF p_reservation_id IS NOT NULL THEN
    ...
    IF p_decision_id IS NOT NULL AND p_next_state IN (
         'ORDER_SUBMITTING','ORDER_SUBMITTED','PENDING_RECONCILIATION',
         'COMPLETED','CANCELLED_UNFILLED'
       ) THEN
...
CREATE OR REPLACE FUNCTION public.p1_advance_automation_checkpoint_v3(p_run_id text, p_claim_token_hash text, p_tick_identity_hash text, p_expected_version integer, p_next_state text, p_selected_symbol text, p_selected_side text, p_decision_id text, p_vertex_call_count integer, p_provider_call_count integer, p_logical_submit_count integer, p_reservation_id text, p_quantity bigint, p_limit_price_krw bigint, p_exact_intent_json text, p_exact_intent_sha256 text, p_quote_snapshot_json text, p_policy_id text, p_policy_version integer, p_position_expiry_session date, p_filled_quantity bigint, p_leaves_quantity bigint, p_unfilled_terminated_quantity bigint, p_average_fill_price_krw bigint, p_exit_reason text, p_expected_account_digest text, p_order_id text, p_provider_order_ref_hash text, p_result_hash text, p_event_type text, p_event_payload_hash text, p_position_state_json text)
 RETURNS TABLE(checkpoint_version integer, replayed boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog'
...
BEGIN
  ...
  IF state_count<>(
    ...
    SELECT 1 FROM jsonb_array_elements(p_position_state_json::jsonb) item
    JOIN public.automation_positions_effective position ON position.position_id=item->>'positionId'
  ...
  IF p_exit_reason='ATR_TRAILING' THEN
    ...
    WHERE user_id=claim_row.user_id AND symbol=p_selected_symbol
      AND account_id=(SELECT account_id FROM public.automation_control WHERE user_id=claim_row.user_id)
