...
CREATE FUNCTION public.p1_sync_completed_automation_order_v1(p_run text,p_claim_hash text,p_exec_hash text)
...
BEGIN
 ...
 IF stored.user_id<>claim.user_id OR stored.brokerage_mode<>'KIS_MOCK'
    ...
    OR stored.provider_order_ref_hash IS DISTINCT FROM reservation.provider_order_ref_hash THEN
