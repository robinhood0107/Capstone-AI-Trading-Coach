-- 체결된 주문이 마감에 닫히지 못하던 결함을 닫는다.
--
-- 2026-09-16, 45주 중 27주가 체결된 주문이 15:20 정산에서 40001
-- `automation exact Decision intent mismatch` 로 막혔다. 다섯 조건 중 걸린 것은
-- `decision_row.valid_until<=statement_timestamp()` 하나다. Decision 의 유효기간은 약
-- 30초인데 정산은 그로부터 여섯 시간 뒤다. 즉 이것은 이 하루의 사고가 아니라, 체결된
-- 주문이라면 언제나 닫히지 못하는 구조였다.
--
-- 가드의 주석은 이미 "판단 결속은 주문을 여는 전이에서만 요구한다" 고 적고 있는데,
-- 상태 목록에는 COMPLETED·CANCELLED_UNFILLED·PENDING_RECONCILIATION 이 들어 있었다.
-- 이들은 주문을 여는 전이가 아니라 이미 나간 주문을 닫는 전이다. 코드를 그 주석에 맞춘다.
--
-- 인가가 약해지지 않는 이유: 이 가드가 막는 것은 "만료된 판단으로 새 주문을 내는 것"이다.
-- 닫는 전이는 무엇도 사지 못한다 - 취소하고, 이미 체결된 수량을 기록할 뿐이다. 그리고
-- 주문 의도 자체의 검증(`automation exact order intent invalid`, 수량 보존, sha256 대조)은
-- 시간과 무관하게 모든 예약 전이에서 그대로 돈다.
--
-- 정의는 운영 중인 것을 그대로 옮기고 위 목록 두 줄만 바꿨다. `CREATE OR REPLACE` 는
-- SET 절을 갈아치우므로 `SET search_path TO 'pg_catalog'` 를 함께 다시 적는다.

CREATE OR REPLACE FUNCTION public.p1_advance_automation_checkpoint_v2(p_run_id text, p_claim_token_hash text, p_tick_identity_hash text, p_expected_version integer, p_next_state text, p_selected_symbol text, p_selected_side text, p_decision_id text, p_vertex_call_count integer, p_provider_call_count integer, p_logical_submit_count integer, p_reservation_id text, p_quantity bigint, p_limit_price_krw bigint, p_exact_intent_json text, p_exact_intent_sha256 text, p_quote_snapshot_json text, p_policy_id text, p_policy_version integer, p_position_expiry_session date, p_filled_quantity bigint, p_leaves_quantity bigint, p_unfilled_terminated_quantity bigint, p_average_fill_price_krw bigint, p_exit_reason text, p_expected_account_digest text, p_order_id text, p_provider_order_ref_hash text, p_result_hash text, p_event_type text, p_event_payload_hash text)
...
BEGIN
  ...
  IF p_reservation_id IS NOT NULL THEN
    ...
    END IF;
    -- 판단 결속은 주문을 여는 전이에서만 요구한다. 위험 판정이 HOLD여서 건너뛰는 전이도
    -- 예약을 들고 오므로, 예약 유무만 보면 정상적인 거절을 기록할 수 없다.
    IF p_decision_id IS NOT NULL AND p_next_state IN (
  ...
  IF p_next_state='EXIT_SELECTED' AND p_selected_side='SELL' THEN
    ...
    WHERE user_id=claim_row.user_id AND account_id=control_row.account_id AND symbol=p_selected_symbol AND status='OPEN';
  ...
  ELSIF p_selected_side='SELL' AND p_next_state IN (
    ...
    WHERE user_id=claim_row.user_id AND account_id=control_row.account_id AND symbol=p_selected_symbol AND status='EXIT_PENDING';
  END IF;
