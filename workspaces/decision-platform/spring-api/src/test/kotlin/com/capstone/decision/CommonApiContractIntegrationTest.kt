...
// 왜: DB/Kafka 없이도 S0.3 REST 공통 계약과 security filter 동작을 빠르게 검증한다.
@SpringBootTest(
    properties = [
...
class CommonApiContractIntegrationTest(
    ...
    @BeforeEach
    fun setUpMockMvc() {
        // 왜: Security filter chain까지 태워야 401/403 envelope와 requestId header를 실제처럼 검증한다.
        mockMvc =
            MockMvcBuilders
    ...
    // 왜: DTO 검증 실패가 Spring 기본 오류가 아니라 VALIDATION_ERROR envelope로 내려가는지 잠근다.
    @Test
    ...
    // 왜: system health는 smoke용 API지만 인증 경계를 우회하면 안 된다.
    @Test
    ...
    // 왜: 로그인 토큰, requestId 보존, health envelope를 한 번에 확인하는 대표 happy path다.
    @Test
    ...
    // 왜: ADMIN 전용 endpoint가 USER 토큰으로 막혀야 이후 운영 API 권한 경계가 흔들리지 않는다.
    @Test
    ...
    // 왜: 존재하지 않는 API도 프론트가 분기 가능한 NOT_FOUND envelope를 받아야 한다.
    @Test
    ...
    // 왜: dashboard 개발 서버가 Authorization/idempotency header를 보낼 수 있어야 수동 smoke가 가능하다.
    @Test
    ...
    private fun login(
        ...
        // 왜: 테스트마다 토큰 발급 흐름을 실제 login API로 통과시켜 JWT 생성 계약도 함께 검증한다.
        val response =
            mockMvc
                .post("/api/v1/auth/login") {
                    contentType = MediaType.APPLICATION_JSON
...
private class TestOnlyAdminController {
    // 왜: production API를 오염시키지 않고 403 권한 경계만 검증하기 위한 test-only endpoint다.
