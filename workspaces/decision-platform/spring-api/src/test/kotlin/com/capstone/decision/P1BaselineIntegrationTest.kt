...
class P1BaselineIntegrationTest {
    ...
    @Test
    fun `fresh database applies B86 through V117 while existing database applies V1 through V117`() {
        assertEquals(
            listOf(
                "86" to "SQL_BASELINE",
                "87" to "SQL",
        ...
        assertEquals(117, history(HISTORICAL_DB).size)
        assertEquals("117" to "SQL", history(HISTORICAL_DB).last())
        assertEquals("117" to "SQL", history(UPGRADE_DB).last())
