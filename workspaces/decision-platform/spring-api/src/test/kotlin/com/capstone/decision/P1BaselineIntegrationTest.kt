...
class P1BaselineIntegrationTest {
    ...
    @Test
    fun `fresh database applies B86 through V88 while existing database applies V1 through V88`() {
        assertEquals(listOf("86" to "SQL_BASELINE", "87" to "SQL", "88" to "SQL"), history(BASELINE_DB))
        assertEquals(88, history(HISTORICAL_DB).size)
        assertEquals("88" to "SQL", history(HISTORICAL_DB).last())
        assertEquals("88" to "SQL", history(UPGRADE_DB).last())
