...
export function Numeric({ value, format, missingReason, className = '' }: NumericProps) {
  if (value === null || !Number.isFinite(value)) {
    return (
      <span
        className={`hatch inline-flex items-center rounded-[1px] px-2 py-0.5 font-mono text-[12px] text-faint ${className}`}
  ...
  return <span className={`tnum font-mono ${className}`}>{format(value)}</span>;
}
...
export function Delta({ value, format }: { value: number | null; format: (v: number) => string }) {
  ...
  return <span className={`tnum font-mono ${tone}`}>{format(value)}</span>;
}
